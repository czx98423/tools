(function (window, undefined) {
        ...
        getData: function () {
            ...
            this.box.forEach(function (data) {
                ...
                if (data == self.parentNode) {
                    ...
                    var json = make.Default.toJson(data)
                    json.position[2] = data.getClient('z') || 0;
                    result.push(json);
                }
            });
            return result;
    ...
    twaver.Util.ext(nsp.edit.BoxBridge, Object, {
        ...
        _removeJsonData: function (data) {
            if (data != null) {
                //在2d里移除的为门洞，要转换为相应的门对象
                if(!this.jsonBox.getDataById(data.getId())){
                    if(data.getClient('doorId')){
                        this.jsonBox.removeById(data.getClient('doorId'));
                    }
                }else{
        ...
        handleOffsetAndLenChange: function(prop,data,childData,newValue){
            if (prop == 'edgeIndex' || prop == 'offset') {
                childData.client = childData.client || {};
                childData.client[prop] = newValue;
            }
            if (prop == 'length') {
        ...
        _handle2dDataChange: function (e) {
            ...
            if (data.getParent() && (data.getClient('type') == "door" || data.getClient('type') == "window")) {
                ...
                //childData可能有多个，拖动门洞时让门也跟着动
                var childrenData = [];
                jsonData.getChildren().forEach(function (child) {
                    if (child.getObjectId() == data.getId()) {
                        childrenData.push(child);
                        if(child.doorId){
                            var doorData = self.jsonBox.getDataById(child.doorId);
                            if(doorData) childrenData.push(doorData);
                        }
                    }
                });
                if (property.startsWith('C:')) {
                    ...
                    childrenData.forEach(function(child){
                        self.handleOffsetAndLenChange(p,data,child,newValue);
                    });
        ...
        _add2dData: function (data) {
            ...
            if (this.synchronizeFilter(null, data)) {
                ...
                if (!this.box2d.getDataById(data.getObjectId())) {
                    ...
                    json.client.label&&node.setClient('bid', json.client.label);
                    this.box2d.add(node);
    ...
    twaver.Util.ext(nsp.edit.Edit2D, Object, {
        _init: function () {
            ...
            var bridge = this.bridge = new nsp.edit.BoxBridge(this.box, this.jsonBox);
            bridge.bind();
            //register image
            //this._registerImage();
            //this.addPropertySheetListener();
        ...
        handleSelectionChange: function () {
            ...
            if (selection.size() != 1) {
                ...
                //当点击的目标为门洞时，将id换成对应的门模型
                if(id.indexOf('twaver.idc.cutoff') != -1){
                    var doorId = node.getClient('doorId');
                    if(doorId){
                        var doorObj = this.jsonBox.getDataById(doorId);
                        if(doorObj && doorObj.id){
                            id = doorObj.id;
                        }
                    }
                }
        ...
        refreshNetwork3d: function () {
            ...
            var object3ds = make.Default.load(datas, function (object3ds) {
                if (object3ds && object3ds.length > 0) {
                    ...
                    object3ds.forEach(function(object){
                        if(object._clientMap.oldParentId){
                            var parentId = object._clientMap.oldParentId;
                            parent = object3ds.filter(function(ob){return ob._clientMap.bid == parentId});
                            if(parent && parent.length ==1){
                                parent = parent[0];
                                object.setParent(parent);
                                var pos = object.getPosition(),
                                    p_pos = parent.getPosition();
                                object.setPositionX(pos.x - p_pos.x);
                                object.setPositionZ(pos.z - p_pos.z);
                            }else{
                                console.log('error data' + parentId);
                            }
                        }
                    })
    ...
    twaver.Util.ext(nsp.edit.ImportDxf, Object, {
        ...
        createJsonArray: function (entity, jsons, id, texts) {
            ...
            // console.log(points);
            var json = {};
            if (type == 'wall' || type == 'innerWall' || type == 'floor' || type == 'area' || type == '0') {
                json = {
                    id: id,
                    data: points,
                }
    ...
    twaver.Util.ext(nsp.edit.JsonBox, Object, {
        add: function (data) {
            ...
            //如果data是门，则不触发2d的事件监听
            if(make.Default.getOtherParameter(data.id,'type') != 'door'){
              sceneEditor.edit2d.bridge._handleJsonBoxChange({
                    kind: 'add',
                    data: data,
                    source: this
                });
    ...
    SceneEditor.prototype = {
        ...
        loadDxfFile: function (text, filter, autoCreateId) {
            ...
            var importDxf = new nsp.edit.ImportDxf(1);
    ...
    twaver.Util.ext($JsonObject, twaver.PropertyChangeDispatcher, {
        ...
        _serializeJsonCommon: function (newInstance, dataObject) {
            for(var p in newInstance){
                if(p == '_dispatcher' || !newInstance.hasOwnProperty(p))  continue;
                if(p == "children"){
                    this.serializeArray(p, newInstance, dataObject);
                }else{
                    this.serializePropertyJson(p, newInstance, dataObject);
                }
            }
            ...
            // this.serializePropertyJson("cid", newInstance, dataObject);
            // this.serializePropertyJson("doorId", newInstance, dataObject);
            ...
            if (obj.id) {
                if (obj.combo) {
                    ...
                    if (type === 'watercable' || type === 'wall' || (type === 'area' && id2d == 'twaver.idc.area.top') || type === 'innerWall' || type === 'floor') {
                            ...
                            if (elements.get(i) instanceof make.Default.WallShapeNode) {
                                ...
                                if (element && (element instanceof make.Default.WallShapeNode)) {
                                    ...
                                    if (index >= 0) {
                                        ...
                                        //算出的弧度要转化为角度
                                        var rotateY = Math.atan((from.y - to.y) / (from.x - to.x)) * 180 / Math.PI;
                                        // var rotateY = Math.atan((from.y - to.y) / (from.x - to.x));

                                        var newPoint = {};       /*修正门窗吸附后的偏移量*/

                                        //如果墙是斜的就不能直接根据两个端点来设置,而是按相交的点来
                                        ...
                                        // }
                                        var newPoint = getCrossPoint(from,to,point);

                                        function getCrossPoint(p1,p2,p3){
                                            var P = {};
                                        ...
                                        /**
                                         * @todo 给门和门洞设置默认的宽度，按照门的make里的默认参数来
                                        */
                                        var defaultWidth = make.Default.getModelDefaultParametersValues('doubleDoor14');
                                        ...
                                        if (wall) {
                                            ...
                                            //类型为门时，改变导出的结构 [floor(cutoff,cutoff),door1,door2]
                                            if(type == 'door'){
                                                if(Object.keys(it.util.dataTypeMap).indexOf(id) != -1){
                                                    wallChild.rotation = [0, -rotateY, 0];
                                                    wallChild.parentId = wall.getObjectId();

                                                    var cutoff = new nsp.edit.data.JsonObject();
                                                    cutoff.id = 'twaver.idc.cutoff';
                                                    //从后台导入的门，有可能将datatypeId作为id
                                                    if(id.indexOf('twaver.idc.door') != -1){
                                                        cutoff.cid = id;
                                                    }else{
                                                        var datatype = it.util.dataTypeMap[id];
                                                        var modelId = datatype['model'] ? datatype['model'] : id;
                                                        cutoff.cid = modelId;
                                                    }
                                                    //给门洞设置门的id
                                                    var dId = wallChild.getObjectId();
                                                    cutoff.doorId = dId;
                                                    cutoff.position = [newPoint.x, 0, newPoint.y];
                                                    cutoff.client = { edgeIndex: index, offset: offset };
                                                    cutoff.rotation = [0, -rotateY, 0];
                                                    cutoff.setWidth(205);
                                                    children.push(cutoff);
                                                    this.jsonBox.add(wallChild);
                                                }else{
                                                    //给旧门的client对象里添加rotation信息，便于保存时创建新门的rotation
                                                    wallChild.client.rotationY = rotateY;
                                                    // wallChild.rotation = [0, -rotateY, 0];
                        ...
                        //如果是floor则添加data属性
                        if(make.Default.getOtherParameter(id, 'sdkCategory') == 'floor'){
                            var defaultDataValue = make.Default.getModelDefaultParametersValues(id);
                            var floorData = defaultDataValue ? defaultDataValue.data : [];
                            jsonData.data = floorData;
                        }
