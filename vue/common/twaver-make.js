    ...
    make.Default = {
        ...
        load: function (object, callback, options) {
            ...
            if (object instanceof Array) {
                ...
                var total = 0;
                if (callback) {
                    var wraperCallback = function (result, data,index) {
                        var length = object.length;
                        return (function (data) {
                            result.datas = result.datas || [];
                            // result.datas.push(data);
                            result.datas[index] = data;
                    ...
                    result._total = 0;
                    var loadChild = function(child,index){
                        self.load(child, function (obj1) {
                            result._total++;
                            wraperCallback(result, obj1, index);
                        });
                    }
...
make.Utils2D = {
    ...
    createFollower: function (json) {
        ...
        if(json.labelFont){
            node.setStyle('label.font', json.labelFont);
        }
        if (json.labelMaxlength) {
            node.setStyle('label.maxlength', parseInt(json.labelMaxlength));
        }
...
make.Utils3D = {
    ...
    getEnvMap: function (id) {
        if (Object.keys(make.Default._envMaps).indexOf(id) != -1) {
    ...
    cloneObjElement: function (json, callback) {
        ...
        // 单例直接返回，若是再执行下面的方法时会将已经设置好的信息改的乱七八糟 2018-05-14 Kevin
        if (json.singleton) {
            var newObject = object;
            if (callback) callback(newObject);
            return newObject;
        }
        var newObject = object.clonePrefab();
        /*
	    var newObject =  object;
        if (!json.singleton) { //billboard孩子的处理
            // if (json.lod) { // lod是billboard
                newObject = object.clonePrefab(false);
                object._childList.forEach(function(child) {
                    if (child instanceof mono.Billboard) {
                        var newChild = new mono.Billboard();
                        var styleMap = child.styleMap;
                        var _client = child._clientMap;
                        for(var key in styleMap){
                            newChild.setStyle(key,styleMap[key]);
                        }
                        for(var ck in _client){
                            newChild.setClient(ck,_client[ck]);
                        }
                        newChild.setScale(child.getScale().clone());
                        newChild.setPosition(child.getPosition().clone());
                        newObject.addChild(newChild);
                        // newObject.addChild(child.clone());
                    }else{
                        newObject.addChild(child.clonePrefab());
                    }
                });
            // }
        }
       ...
       newObject.setClient('type', json.name);
        ...
        // 如果是单例的话，第一个callback中可能已经设置好值了，接下来不管有没有callback再这么设置会把之前的值覆盖掉
    ...
    attachLod : function(json,complexNode){
        var lod = json.lod;
        if (lod && lod.id) {
            make.Default.load(json.lod.id,function(simpleNode){
                simpleNode.setParent(complexNode);
                if (lod.scale) {
                    simpleNode.setScale(lod.scale[0], lod.scale[1], 1);   
                }
                if (lod.position) {
                    simpleNode.setPosition(lod.position[0], lod.position[1], lod.position[2]);
                }
                simpleNode.setClient('simple', true);
                simpleNode.setClient('lod', true);
                complexNode.setClient('lod', true);
            });
        }
    },

    setMtypeAnimate:function(obj,animation){
        var self = this;
        if (animation && animation.length) {                           
            obj.getDescendants().forEach(function(desc){
                var mname = desc.mname;
                animation.forEach(function(a){
                    if(a.mname == mname){
                        desc.setClient('animation', a.animate);
                        if(a.mtype){
                            desc.setClient('mtype', a.mtype);
                        }
                        if(a.animateType){
                            desc.setClient('animateType', a.animateType);
                        }
                    }
                });
                self.setMtypeAnimate(desc,animation);
            });
        }     
    },

    setMtypeEnvImage: function(obj, items, envImg) {
        var self = this;
        if (!items) return;
        obj.getDescendants().forEach(function(desc){
            var mname = desc.mname;
            var image = desc.getStyle('m.texture.image');
            if (Array.isArray(items)) {
                items.forEach(function(item){
                    if ((image && (image.indexOf(item) != -1 || (image instanceof Array && image[0].indexOf(item) != -1))) || item.indexOf(mname) != -1) {
                        desc.setStyle('m.envmap.image', envImg);
                    }
                });
            } else if ($.isPlainObject(items)) {
                for (var i in items) {
                    var newEnvImg = make.Default.getEnvMap(items[i]);
                    if ((image && (image.indexOf(i) != -1 || (image instanceof Array && image[0].indexOf(i) != -1))) || mname == i) {
                        desc.setStyle('m.envmap.image', newEnvImg);
                    }
                }
            }
            self.setMtypeEnvImage(desc, items, envImg);
        });
    },
    ...
    createObj: function(json, callback) {
        ...
        if (this._objObject[json.name]) {
            ...
            var objElement = this.cloneObjElement(json, callback);
            // this.attachLod(json,objElement);
            return objElement;
        } else {
            if (!this._objMap[json.name] || this._objMap[json.name].length < 1) {
                ...
                var loadObj = function() {
                    ...
                    loader.load(obj, mtl, null, function(object) {
                        if (json.merged) {
                            ...
                            //哪些面需要环境贴图
                            // 生成环境贴图的方式，识别子对象的材质名或贴图
                            var envOriginImages = json.envOriginImages;
                            var envmap = json.envImage ? make.Default.getEnvMap(json.envImage) : make.Default.getEnvMap("envmap5");
                            self.setMtypeEnvImage(object, envOriginImages, envmap);
                            //设置子对象动画
                            self.setMtypeAnimate(object,animation);
                        }
                        
                        self.attachLod(json,object);
                        //设置object oid
                ...
                if (!!json.isGltf) {
                    mono.GLTFLoader.load(objPath, json.name, function(object) {
                        if (json.merged) {
                            if (!json.isBufferNode) {
                                object = mono.Utils.mergeElements(object.getDescendants());
                            }
                            object.getDescendants().forEach(function(descendant) {
                                if (descendant._name) descendant.setClient('oid', descendant._name);
                        ...
                        self._objMap[json.name].forEach(function(j) {
                            if (!json.isBufferNode) {
                                return self.cloneObjElement(j.json, j.callback); // bufferNode没法clonePrefab
                            } else {
                                j.callback && j.callback(object);
                            }
                        });
                    },
                    // function() {
    ...
    generateTextCanvas: function (text, color, font, fillbg, bgcolor, hideShadow) {
        var canvas = document.createElement('canvas');
        var font = font || "bold 80px 微软雅黑,sans-serif";

        var context = canvas.getContext('2d');
        var values = text.split('\n');
        var size = make.Default.getTextSize(text, font);
        var w = size.w,
            h = size.h;
        ...
        if (fillbg) {
            ...
            context.fillStyle = bgcolor || '#FE642E';
            context.beginPath();
            context.moveTo(0, 0);
            context.lineTo(w, 0);
            context.lineTo(w, h);
        ...
        context.font = font;
        if (!hideShadow) {
            context.shadowColor = "#585858";
            context.shadowBlur = 25;
            context.shadowOffsetX = 10;
            context.shadowOffsetY = 10;
        }
        var length = context.measureText("o").width;
        var rowHeight = length * 2.5
        for (var i = 0; i < values.length; i++) {
            var x = w / 2;
            var y = rowHeight * i + rowHeight / 2 + (canvas.height - drawRect.height)/2;
            context.fillText(values[i], x, y);
        }
    ...
    generateTextOrImgCanvas: function (text, color, font, bgcolor, labelOpacity,img) {
        ...
        //绘制背景
        outerCxt.fillStyle = bgcolor || 'transparent';
    ...
    createShadowImage: function (wall, floor, visibleFilter, shadowParams) {
        ...
        if (columnCombo) {
            ...
            combos.forEach(function (child) {
                ...
                if (child.getClient && !child.getClient('resetPos')) {
                    var pos = child.getPosition();
                    var ccbPos = columnCombo.getWorldPosition();
                    ccbPos = ccbPos.clone().sub(wallPosition);
                    child.p(ccbPos.x+pos.x + parseFloat(wallPosition.x), ccbPos.y+pos.y + parseFloat(wallPosition.y), ccbPos.z+pos.z + parseFloat(wallPosition.z));
                    child.setClient('resetPos', true);
                }
                descendants.push(child);
            });
            ...
            if (showShadow) {
                ...
                if (shadowImage) {
                    ...
                    if (shadowType == 0) { //矩形
                        ...
                        if (radius) {
                            radius = radius / scaleH * data.getScaleZ();
                        } else {
    ...
    playAnimation: function (element, animation, done) {
        ...
        if(params[0] === 'twinkle'){
            var dur = parseInt(params[1]) || 1000;
            var delay = parseInt(params[2] || 0);
            var easing = params[3];
            return this.animateTwinkle(element,dur, delay, easing);
        }
    },

    animateTwinkle:function(element,dur, delay, easing){
        var animation = new twaver.Animate({
            ...
            onUpdate: function (value) {
                if(value > 0.5){
                    element.setStyle('m.opacity',(value-0.5)*2);
                }else{
                    element.setStyle('m.opacity',(0.5-value)*2);
                }
            }
        });
        animation.play();
    ...
    animateRotate: function (object, anchor, angle, dur, delay, easing, done) {
        ...
        easing = easing || 'easeInStrong';
        var center = object.boundingBox.center() || new mono.Vec3(0, 0, 0);
        var size = object.getBoundingBox().size().multiply(object.getScale());
        ...
        if (anchor === 'top') {
            if(center){
                position = new mono.Vec3(center.x, center.y +size.y / 2, center.z);
            }else{
                position = new mono.Vec3(0, size.y / 2, 0);
            }
        ...
        if (anchor === 'left') {
            if(center){
                position = new mono.Vec3(center.x-size.x / 2 / object.getScale().x, center.y / object.getScale().y, center.z / object.getScale().z);
            }else{
        ...
        if (anchor === 'front') {
            if(center){
                position = new mono.Vec3(center.x / object.getScale().x, center.y / object.getScale().y, center.z - size.z / 2 / object.getScale().z);
            }else{
                position = new mono.Vec3(0, 0, -size.z/2);
            }
            var axis = new mono.Vec3(0, 1, 0);
        }
        if (anchor === 'back') {
            if(center){
                ...
                position = new mono.Vec3(0, 0, size/2);
            }
            var axis = new mono.Vec3(0, 1, 0);
        }
        if (anchor === 'center-z') {
        ...
        if (anchor === 'center-x') {
            ...
            position = center || new mono.Vec3(0, 0, 0);
            var axis = new mono.Vec3(1, 0, 0);
        }
        if (anchor === 'center-y') {
            position = center || new mono.Vec3(0, 0, 0);
...
make.Default.register('twaver.billboard', function (json, callback) {
    var textBillboard = new make.Default.TextBillboardWithArrow();
    delete json.width;
    delete json.height;
    var object = textBillboard.createBillboard(json);
    ...
    return object;
        ...
        description: "广告牌是make里面提供的最基本的模型，可以用这种模型扩展出各种文字的模型。支持更改背景色，文字等基本参数",
...
make.Default.register('twaver.cube', function (json, callback) {
    ...
    var object = make.Default.createCube(json);
      var transparent=json.transparent||false;
    var opacity=json.opacity||1;
    if(json.transparent==true){
        object.setStyle('m.transparent',true);
       object.setStyle('m.opacity',opacity); 
    }
...
make.Default.register('twaver.combo', function (json, callback) {
    ...
    if (data && mono.Utils.isArray(data)) {
        ...
        for (var i = 0; i < data.length; i++) {
            ...
            if (!mainObject) {
                ...
                var type = object.getClient('type');
                 if (type && (type == 'inner_wall' || type == 'glass_wall')) {
        ...
        if (columnCombos && columnCombos.length > 0) {
            ...
            // combo.setClient('combos', combo.getCombos());
            // combo.setParent(mainObject);
            // columnCombos.forEach(e=>{
            //     e.setParent(mainObject);
            // })
            columnCombos.forEach(function(e){
                e.setParent(mainObject);
            })
        }
...
make.Default.registerObj = function (id, name, path, parameters, showShadow, shadowType, merged, showShadowImage, style, envImage, singleton, isAbsPath,isGltf, animation,lod,envOriginImages,flipY) {
    if(typeof id == 'object'){//参数可为字符串集合或对象
        var args = id;
        id = args.id;
        name = args.name;
        path = args.path;
        ...
        merged = args.merged;
        showShadowImage = args.showShadowImage;
        style = args.style;
        envImage = args.envImage;
        envOriginImages = args.envOriginImages;
        singleton = args.singleton;
        isAbsPath = args.isAbsPath;
        isGltf = args.isGltf;
    ...
    var fun = function (json, callback) {
        ...
        json.merged = typeof json.merged == 'boolean' ? json.merged : merged;
        json.envImage = envImage;
        json.envOriginImages = envOriginImages;
        ...
        json.lod = lod;
        json.flipY = flipY;
        json.animation = json.animation ? json.animation : animation;
...
make.Default.TextBillboardWithArrow = function () {

	// 默认的参数
	this.position = [0, 0, 0];
	this.scale = [0.3, 0.3, 1];
	this.text = '测试标题\n测试内容';
	this.ifHasTitle = 'true';
	this.isHasSeparator = 'true';
	this.lineSeparator = '：';
	this.bgColor = 'rgba(26,50,72,0.8)';
	this.borderColor = 'rgba(0, 233, 255)';
	this.contentColor = '#FFFFFF';
	this.globalAlpha = 0.9;
	// this.arrowCut = 2;
	this.arrowPosition = 'noArrow';
    this.triangleTipPosition = null;
    // this.cornerArc = 1;
    this.triangleBorder = true;
    // this.downmostBgColor = 'rgba(5,19,30,0.3)';
    // this.paddingSize = 2;
};

twaver.Util.ext('make.Default.TextBillboardWithArrow', Object, {
	 ...
	 * @param {string} options.cornerArc - billboard中的面板转角的弧度，默认值1，值越大，弧度越大
	 * @returns 
	 */
	createBillboard: function (options) {
		...
		var billboard = this.makeBillboardByCanvas(canvas);
		// position
	...
	makeBillboardByCanvas : function (canvas) {
	    ...
	    canvas.width = bigCanvasWidth;
	    canvas.height = bigCanvasHeight;
	    ctx.clearRect(0, 0, bigCanvasWidth, bigCanvasHeight);
	    ctx.drawImage(newCanvas, 0, bigCanvasHeight - canvasHeight, canvasWidth, canvasHeight);
	...
	getTextBillboardCanvas: function (options) {
		...
		var ifHasTitle = typeof options.ifHasTitle == 'boolean' ? options.ifHasTitle.toString() : (options.ifHasTitle || this.ifHasTitle);
		var ifHastwoTitle = options.ifHastwoTitle;
		var twoTitleFont = options.twoTitleFont || '160px';
		var isHasSeparator = typeof options.isHasSeparator == 'boolean' ? options.isHasSeparator.toString() : (options.isHasSeparator || this.isHasSeparator);
        var lineSeparator = options.lineSeparator || this.lineSeparator;
        var cornerArc = options.cornerArc||this.cornerArc;
		...
		canvas.width = width?width:content_width;
		canvas.height = height?height:content_height;
		var lineHeight = txt_height / array.length;
		var lineHeightSpace;

		options = make.Default.ext({
			width: content_width,
			height: content_height,
			radius: (content_width / 64)*cornerArc,
		...
		if(firstLine){
			...
			if(ifHastwoTitle){
				...
				context.fillStyle = contentColor || '#fff';
				context.fillText(text, lineWidth + txt_space, lineWidth + lineHeight / 2 + txt_space);
				context.font = "bolder " +twoTitleFont+ " 'Source Han Sans CN', 'Microsoft YaHei', Verdana, Helvetica, Arial, 'Open Sans', sans-serif";
				context.fillStyle = '#fff';
				context.fillText(unit, lineWidth + txt_space+350, lineWidth + lineHeight / 2 + txt_space);
			}else {
				context.fillStyle = contentColor || '#fff';
	...
	getTextBox: function (options) {
        ...
        var triangleBorder = options.triangleBorder || this.triangleBorder;
        ...
        var downmostBgColor = options.downmostBgColor||this.downmostBgColor;
        var paddingSize = options.paddingSize||this.paddingSize;
		...
		if (arrowPosition == 'right') {
			...
			context.lineTo(width * arrowCut / 4 + 50, height - arrowHeight - lineWidth);
			context.lineTo(width * arrowCut / 4, height - arrowHeight - lineWidth + 70);
			context.lineTo(width * arrowCut / 4 - 50, height - arrowHeight - lineWidth);
            ...
            if(!downmostBgColor){
                ...
                context.fillStyle = downmostBgColor;
                context.lineWidth = 1.4*lineWidth;
                context.save();
                context.moveTo(radius + lineWidth, lineWidth);
                context.lineTo(width - radius - lineWidth, lineWidth);
                context.arcTo(width - lineWidth, lineWidth, width - lineWidth, radius + lineWidth, radius);
                ...
                paddingSize = paddingSize * radius;
                context.fillStyle = bgColor;
                context.lineWidth = lineWidth;
                context.save();
                context.beginPath();
                context.strokeStyle = 'transparent';
                context.moveTo(radius + lineWidth + paddingSize, lineWidth + paddingSize);
                context.lineTo(width - radius - lineWidth - paddingSize, lineWidth+ paddingSize);
                context.arcTo(width - lineWidth - paddingSize, lineWidth+ paddingSize, width - lineWidth - paddingSize, radius + lineWidth + paddingSize, radius);
                context.lineTo(width - lineWidth - paddingSize, height - radius - lineWidth - paddingSize);
                context.arcTo(width - lineWidth - paddingSize, height - lineWidth - paddingSize, width - radius - lineWidth - paddingSize, height - lineWidth - paddingSize, radius);
                context.lineTo(radius + lineWidth + paddingSize, height - lineWidth - paddingSize);
                context.arcTo(lineWidth + paddingSize, height - lineWidth- paddingSize, lineWidth- paddingSize + paddingSize, height - radius - lineWidth- paddingSize, radius);
                context.lineTo(lineWidth + paddingSize, radius + lineWidth + paddingSize);
                context.arcTo(lineWidth + paddingSize, lineWidth + paddingSize, radius + lineWidth + paddingSize, lineWidth + paddingSize, radius);
            }
		} else {
        ...
        if (triangleBorder) {
			context.strokeStyle = borderColor;
            context.lineWidth = lineWidth * 2 + radius * 2;
            var len = 6;
            context.moveTo(radius*len, 0);
            context.lineTo(0, 0);
            context.lineTo(0, radius*len);

            context.moveTo(width - radius*len, 0);
            context.lineTo(width, 0);
            context.lineTo(width, radius*len);

            context.moveTo(width, height - radius*len);
            context.lineTo(width, height);
            context.lineTo(width - radius*len, height);

            context.moveTo(radius*len, height);
            context.lineTo(0, height);
            context.lineTo(0, height - radius*len);
            context.stroke();
        }
		context.restore();
		return canvas;
...
make.Default.register('twaver.idc.wall.top', function (json) {
    ...
    if (children) {
        children.forEach(function (child) {
            ...
            childNode && childNode.setParent(wall);
        });
    }
    return wall;
...
make.Default.register('twaver.idc.innerWall.top', function (json) {
    ...
    if (children) {
        children.forEach(function (child) {
            var childNode = make.Default.load(child);
            childNode && childNode.setParent(innerWall);
        });
    }
    return innerWall;
...
make.Default.register('twaver.idc.door.top', function (json) {
    ...
    //门洞在2d里用的也是door的模型，这里把doorId保存在client属性里方便以后使用
    if(json.doorId){
        doorJson.client.doorId = json.doorId;
    }
...
make.Default.copy('twaver.idc.cutoff.top', 'twaver.idc.door.top', {}, {
    name: "墙洞",
    description: "墙洞",
    ...
    var vectorImage = {
        ...
        v: [{
            ...
            draw: function (g, data, view) {
                ...
                var rackTextWrap = data.getClient('channel.rackText.wrap');
                if (rackTextWrap == null) {
                    rackTextWrap = true;
                }
                var textRotate = data.getClient('channel.rackText.rotate')||0;
                ...
                function drawRack(x, y, rackWidth, rackDepth, rackText) {
                    ...
                    g.drawShape({
                        ...
                        font: rackTextFont,
                        fill: rackTextColor,
                        w: rackTextWrap?textWidthMax:null,
                        rotate:textRotate,
                        x: x + rackWidth / 2,
                        y: y + rackDepth / 2,
                        rotateOrigin:{ x: x + rackWidth / 2, y: y +rackDepth / 2 }
                    });
                }
...
make.Default.register('0.top', function (json) {
  var wallJson = {
      objType: 'wall',
      client: {
          'data': json.data,
...
make.Default.copy('twaver.idc.wall15.top', 'twaver.idc.wall.top', {}, { icon: getIdcIconPath('wall14.png') });
make.Default.copy('twaver.idc.wall16.top', 'twaver.idc.wall.top', {}, { icon: getIdcIconPath('wall14.png') });
make.Default.copy('twaver.idc.wall18.top', 'twaver.idc.wall.top', {}, { icon: getIdcIconPath('wall14.png') });
make.Default.copy('twaver.idc.wall19.top', 'twaver.idc.wall.top', {}, { icon: getIdcIconPath('wall14.png') });
...
make.Default.copy('twaver.idc.innerWall16.top', 'twaver.idc.innerWall.top', {}, { icon: getIdcIconPath('innerWall15.png') });
make.Default.copy('twaver.idc.innerWall17.top', 'twaver.idc.innerWall.top', {}, { icon: getIdcIconPath('innerWall15.png') });
make.Default.copy('twaver.idc.innerWall18.top', 'twaver.idc.innerWall.top', {}, { icon: getIdcIconPath('innerWall10.png') });
make.Default.copy('twaver.idc.innerWall19.top', 'twaver.idc.innerWall.top', {}, { icon: getIdcIconPath('innerWall10.png') });
...
make.Default.copy('twaver.idc.glassWall9.top', 'twaver.idc.glassWall.top', {}, { icon: getIdcIconPath('glassWall8.png'), });
...
make.Default.copy('twaver.idc.column18.top', 'twaver.idc.column.top', {}, { icon: getIdcIconPath('column13.png') });
make.Default.copy('twaver.idc.column19.top', 'twaver.idc.column.top', {}, { icon: getIdcIconPath('column10.png') });
...
make.Default.register('twaver.idc.floor', function (json, callback) {
    ...
    var getCutObject = function (data) {
        var cutObject = {
            type: 'shapeNode',
            data: data,
            vertical: true,
    ...
    //切割floor ---add by yxk
    var cutData = json.cutData;
    if (cutData && cutData.length > 0) {
        var cutObjects = [];
        if (cutData[0][0] instanceof Array) {
            for (var k = 0; k < cutData.length; k++) {
                cutObjects.push(getCutObject(cutData[k]));
            }
        } else {
            cutObjects.push(getCutObject(cutData));
        }

        var cutNode = make.Default.createCombo(cutObjects);
        floor = new mono.ComboNode([floor, cutNode], ['-'])
    }
    floor.setClient('type', 'floor');
...
make.Default.register('twaver.idc.roof', function (json, callback) {
    ...
    var gap = 400;
    var repeat = json.repeat || [width / gap, depth / gap];
    roof.s({
        'm.transparent': false,
        'm.alphaTest': 0.5,
        'm.texture.image': pic,
...
make.Default.register('twaver.idc.wall', function (json, callback) {
    ...
    var wallHeight = json.wallHeight;
    ...
    var tpInsideImage = json.tpInsideImage ? getIdcWallImagePath(json.tpInsideImage) : null; //玻璃墙贴图
    var tpOutsideImage = json.tpOutsideImage ? getIdcWallImagePath(json.tpOutsideImage) : null; //玻璃墙贴图
    ...
    if (transparent == true) {
        wall = {
            ...
            insideImage: tpInsideImage,
            outsideImage: tpOutsideImage,
            insideMap: insideMap,
            ...
            data: json.data,
            closed: json.closed,
            // insideImage: 'glass-wall.png',
            // outsideImage: 'glass-wall.png',
            style: {
                'inside.m.type': 'phong',
    ...
    if (json.children) {
        for (var i = 0; i < json.children.length; i++) {
            ...
            var childWidth = json.children[i].width || 160;
            if (json.children[i].id.indexOf('twaver.idc.door') >= 0 || json.children[i].id.indexOf('twaver.idc.cutoff') >= 0) {
                var resId = '';
                if (json.children[i].id.indexOf('twaver.idc.door') >= 0) {
                    resId = json.children[i].id;
                } else if (json.children[i].cid && json.children[i].cid.indexOf('twaver.idc.door') >= 0) {
                    resId = json.children[i].cid;
                } else {
                    continue;
                }
                if (make.Default.getModelDefaultParameters(resId)) {
                    var transparent = make.Default.getModelDefaultParameters(resId).transparent && make.Default.getModelDefaultParameters(resId).transparent.value;
    ...
    if (json.showFloor) {
        var position = json.position || [0, 0, 0];
        var floorImage = json.extend && json.extend.floorImage || json.floorImage || 'floor.jpg';
        var floorColor = json.floorColor;
        var floorStyle = json.floorStyle;
        var floorAmount = json.floorAmount;
        var floorPosition = json.floorPosition;
        var floorRepeat = json.floorRepeat || 183;
        var floorId = json.floorId || 'twaver.idc.floor';
        var floorCutData = json.floorCutData;
        var floorTransparent = json.floorTransparent || false;
        var floorOpacity = json.floorOpacity || 1;
        var floor = make.Default.load({
            ...
            style: floorStyle,
            amount: floorAmount,
            cutData: floorCutData,
            repeat: floorRepeat,
            transparent: floorTransparent,
...
make.Default.register('twaver.idc.innerWall', function (json, callback) {
    ...
    var insideMap = json.insideMap ? make.Default.getImagePath(json.insideMap) : null;
    var outsideMap = json.outsideMap ? make.Default.getImagePath(json.outsideMap) : null;
    var insideImage = json.insideImage ? getIdcWallImagePath(extend && extend.insideImage ? extend.insideImage : json.insideImage) : null;
    var outsideImage = json.outsideImage ? getIdcWallImagePath(extend && extend.outsideImage ? extend.outsideImage : json.outsideImage) : null;
    var position = json.position || [0, 0, 0];
    var showBaseboard = json.showBaseboard;
    var baseBoardHeight = json.baseBoardHeight || 15;
    position[0] = parseFloat(position[0]) || 0;
    position[1] = parseFloat(position[1]) || 0;
    position[2] = parseFloat(position[2]) || 0;
    ...
    var wall = {
        ...
        op: '+',
        width: width,
        height: showBaseboard ? height - baseBoardHeight : height,
        insideColor: json.insideColor,
        outsideColor: json.outsideColor,
        asideColor: json.asideColor,
        zsideColor: json.zsideColor,
        ...
        outsideMap: outsideMap,
        repeat: json.repeat,
        position: [0, showBaseboard ? baseBoardHeight : 0, 0],
        data: json.data,
        // style: style
        style: {
            'aside.m.type': 'basic',
...
var getCommInnerWallParameters = function (params) {
    ...
    if (params.outsideColor != null) result.outsideColor.value = params.outsideColor;
    if (params.insideColor != null) result.insideColor.value = params.insideColor;
    if (params.asideColor != null) result.asideColor.value = params.asideColor;
    if (params.zsideColor != null) result.zsideColor.value = params.zsideColor;
    if (params.topColor != null) result.topColor.value = params.topColor;
    if (params.insideMap != null) result.insideMap.value = params.insideMap;
    if (params.outsideMap != null) result.outsideMap.value = params.outsideMap;
...
make.Default.register('twaver.idc.glassWall', function (json, callback) {
    ...
    var client = json.client || {};
    var width = json.wallDepth;
    var insideImage = getIdcWallImagePath(extend && extend.insideImage ? extend.insideImage : json.insideImage);
    var outsideImage = getIdcWallImagePath(extend && extend.outsideImage ? extend.outsideImage : json.outsideImage);
    var repeat = json.repeat || json.wallHeight;
    var position = json.position || [0, 0, 0];
    var showBaseboard = json.showBaseboard;
    var position = json.position || [0, 0, 0];
    var envMap = json.envMap ? make.Default.getEnvMap('envmap2') : null;
    ...
    var wall = {
        ...
        style: {
            ...
            'inside.m.transparent': true,
            'outside.m.transparent': true,
            'inside.m.envmap.image': envMap,
            'outside.m.envmap.image': envMap,
            'aside.m.type': 'basic',
            'zside.m.type': 'basic',
        },
    };
...
make.Default.register('twaver.idc.door1', function (json, callback) {
    var position = json.position || [0, 0, 0];
    var x = position[0],
        y = position[1],
        z = position[2];
    ...
    var direction = json.direction || 'left'

    if (doorAnimationType == 2) {
        var animateLeft = 'move:left:1:1000:0:bounceOut';
        var animateRight = 'move:right:1:1000:0:bounceOut';
    ...
    if (direction == 'right') {
        direction = animateRight;
    } else { direction = animateLeft; }
    ...
    parts.push({
        ...
        client: {
            ...
            'type': 'door',
            'bid': client.bid,
            'doorId': json.objectId || client.bid
        },
    });
    if (json.inbuilt) {
        return parts;
    ...
    var door = make.Default.createCombo(comboChildren);
    if (returnObjects.length > 0) {
        for (var i = 0; i < returnObjects.length; i++) {
            var returnObject = returnObjects[i];
            var object = make.Default.createCombo(returnObject);
            object.setParent && (object.setParent(door));
        }
    }
    make.Default.setObject3dCSProps(door, json);
    callback && callback(door);
    return door;
}, {
    category: "房间模型",
    type: "door",
...
var getDoorParameters = function () {
    var result = getBasicParameters(160, 180, 26);
    var params = {
        'color': {
            name: '颜色',
            value: '#9E9E9E',
...
make.Default.register('twaver.idc.cutoff', function (json) {
    var parts = [];
    var position = json.position || [0, 0, 0];
    var x = position[0],
        y = position[1],
        z = position[2];
    var rotation = make.Default.getRsByDs(json.rotation) || [0, 0, 0];
    var width = json.width || 160,
        height = json.height || 180,
        depth = json.depth || 26,
        frame = json.frame,
        frameColor = json.frameColor || '#FEFEFE';
    parts.push({
        //door cut off.
        type: 'cube',
        width: width + 1,
...
/**
 * 兼容以前的模式，门和墙在一起处理，放到wall里面处理
 * 现在分两部分，先通过cutoff剪掉部分墙体，然后通过door创建门模型
 */
/**
 * 兼容以前的模式，门和墙在一起处理，放到wall里面处理
 * 现在分两部分，先通过cutoff剪掉部分墙体，然后通过door创建门模型
 */
make.Default.register('twaver.idc.door', function (json, callback) {
    var extend = json.extend;
    var position = json.position || [0, 0, 0];
    var x = position[0],
        y = position[1],
    ...
    var leftImage = getIdcDoorImagePath(extend && extend.leftImage ? extend.leftImage : json.leftImage);
    var rightImage = getIdcDoorImagePath(extend && extend.rightImage ? extend.rightImage : json.rightImage);
    var envmap = json.envmap;
    var transparent = json.transparent || true;
    var parts = [];
    var doorAnimationType = json.doorAnimationType || 1;
    //if (transparent != false) transparent = true;
    var client = json.client || {};
    var type = client.type || 'door';
    if (doorAnimationType == 2) {
        var animateLeft = 'move:left:1:1000:0:bounceOut';
        var animateRight = 'move:right:1:1000:0:bounceOut';
    } else {
    ...
    parts.push({
        ...
        style: {
            'm.type': 'phong',
            'm.transparent': JSON.parse(transparent),
...
make.Default.register('twaver.idc.door5', function (json, callback) {
    ...
    rightDoor.setClient('type', type);
    rightDoor.setClient('doorId', json.objectId || client.bid);
...
var getSingleDoorParameters = function (params) {
    ...
    if (params.direction) result.direction.value = params.direction;
    if (params.frame) result.frame.value = params.frame;
    return result;
}
...
make.Default.register('twaver.idc.window', function (json) {
    ...
    var platformHeight = 5,
        platformDepth = 45,
        platformOffsetZ = 10;
    var image = getIdcWindowImagePath(extend ? extend.image : json.image);
...
make.Default.register('twaver.idc.column', function (json, callback) {
    ...
    var depth = json.depth;
    var color = json.color;
    var image = getIdcImagePath(extend ? extend.image : json.image);
...
var columns = [
    ...
    { id: 'column16', height: 310, topColor: "#6d7080", color: 'white', image: 'column16.jpg', column_lightmap1: 'column_lightmap4.jpg', column_lightmap2: 'column_lightmap4.jpg' },
    { id: 'column17', height: 310, topColor: "#dbdee5", color: 'white', image: 'modellib/model/idc/images/wall/inner-wall17.jpg', column_lightmap1: 'column_lightmap4.jpg', column_lightmap2: 'column_lightmap4.jpg' },
    { id: 'column18', height: 310, topColor: "#6d7080", color: 'white', image: 'column18.jpg', column_lightmap1: 'column_lightmap4.jpg', column_lightmap2: 'column_lightmap4.jpg' },
    { id: 'column19', height: 310, topColor: "#5f677d", image: 'modellib/model/idc/images/wall/inner-wall19.jpg', column_lightmap1: 'column_lightmap4.jpg', column_lightmap2: 'column_lightmap3.jpg' },
...
var getFrontPic = function (height, width, uOrder) {
    ...
    //  支持U位从上至下的顺序 2018.4.8 add by lyz 
    if (uOrder == "upToDown") {
        frontPicMap[42] = 'rack42_u2d.png';
        frontPicMap[43] = 'rack43_u2d.png';
        frontPicMap[44] = 'rack44_u2d.png';
        frontPicMap[45] = 'rack45_u2d.png';
        frontPicMap[46] = 'rack46_u2d.png';
        frontPicMap[47] = 'rack47_u2d.png';
    }
...
make.Default.register('twaver.idc.frame', function (json, callback) {
    ...
    var frameCutDepth = json.frameCutDepth;
    ...
    var cut = new mono.Cube(frameCutWidth, frameHeight, frameCutDepth);
...
make.Default.register('twaver.idc.rack', function (json, callback) {
    ...
    var frameCutDepth = json.frameCutDepth || depth * 0.8;
    ...
    var frontPic = json.frontImage || getFrontPic(height, width, json.uOrder);
    var doorAngle = json.doorAngle;
    var backDoorAngle = json.backDoorAngle;
    var rotationFrontDoor = json.rotationFrontDoor || 'right';
    var rotationBackDoor = json.rotationBackDoor || 'right';
    ...
    var BackAnimationLeft = backDoorAngle && 'rotation:right:{}:1000:0:bounceOut'.format(-backDoorAngle || '135');
    var BackAnimationRight = backDoorAngle && 'rotation:left:{}:1000:0:bounceOut'.format(backDoorAngle || '135');
    ...
    var loadFronDoor=json.loadFronDoor||true;
    var loadBackDoor=json.loadBackDoor||true;
    var doorDepth = 2;
    var offset, cutWidth;
    if (haveFrame) {
        offset = make.Default.RACK_OFFSET_Y;
        cutWidth = frameCutWidth + offsetX;
    } else {
        offset = make.Default.RACK_OFFSET_Y;
  ...
  if (json.doubleFrontDoor) {
        ...
        } else if (rotationFrontDoor == 'right') {
 ...
 if (json.doubleBackDoor) {
        ...
        } else if (rotationBackDoor == 'right') {
...
make.Default.copy('twaver.idc.rack2', 'twaver.idc.rack', function (json, callback) {
    ...
    //  支持U位从上至下的顺序 2018.4.8 add by lyz 
    var frontPicArray = json.type && json.type.split('twaver.idc.rack2-');
    if (frontPicArray && frontPicArray.length == 2) {
        if (json.uOrder == "upToDown") {
            frontPic[frontPicArray[1]] = 'rack' + frontPicArray[1] + '_u2d.png';
        }
    }
...
make.Default.register('twaver.idc.rack4', function (json, callback) {
    ...
    //  支持U位从上至下的顺序 2018.4.8 add by lyz 
    var frontPicArray = json.type && json.type.split('twaver.idc.rack4-');
    if (frontPicArray && frontPicArray.length == 2) {
        if (json.uOrder == "upToDown") {
            frontPic[frontPicArray[1]] = 'rack' + frontPicArray[1] + '_u2d.png';
        }
...
make.Default.register('twaver.idc.area', function (json, callback) {
    ...
    var labelColor = json.labelColor || 'red';
    var labelFont = json.labelFont;
    var labelBg = json.labelBg;
    var labelOpacity = json.labelOpacity || 1;
    var opacity = json.opacity || 1;
    ...
    //新增贴图效果
    if ((label && label.trim()) || (image && image.trim()) || (labelBg && labelBg.trim())) {
        ...
        if (image) {
            ...
            gImage.onload = function () {
                canImg.src = make.Default.generateTextOrImgCanvas(label, labelColor, labelFont, labelBg, labelOpacity, gImage);
            }
            gImage.src = pageConfig.url("/images/" + image);
        } else {
            canImg.src = make.Default.generateTextOrImgCanvas(label, labelColor, labelFont, labelBg, labelOpacity);
        }
        $(canImg).css({
            'width': bWidth + 'px',
            'height': bHeight + 'px',
        ...
        area.setClient('cSticker', labelNode);
    }
    // 根据模型参数创建bb
    if (json.bbdText) {
        var centerX = 0,
            centerZ = 0,
            centerY = json.bbdHeight || 310,
            bbdScale;
        if (area.getBoundingBox()) {
            var centerPos = area.getBoundingBox().center();
            if (centerPos) {
                centerX = centerPos.x;
                centerZ = centerPos.z;
            }
        ...
        try {
            if (!isNaN(json.bbdScale)) {
                bbdScale = new Array(3).fill(json.bbdScale);
            } else {
                bbdScale = JSON.parse(json.bbdScale);
            }
        } catch (e) {
            console.error(e);
        }
        var bbd = make.Default.load({
            id: 'twaver.billboard',
            text: json.bbdText,
            scale: bbdScale || [0.5, 0.5, 0.5],
...
//板卡参数
var getCardParameters = function (param) {
    var result = getBasicParameters(param.width, param.height, param.depth);
    result.path = {
        value: param.path,
        name: "贴图路径",
        type: make.Default.PARAMETER_TYPE_STRING,
...
make.Default.register('twaver.idc.card', function (json, callback) {
    ...
    var rotation = make.Default.getRsByDs(json.rotation) || [0, 0, 0];
    var color = json.color || 'white';
    var pic = json.frontImage || getIdcImagePath(json.image || 'card2.png');
...
var getPdu2Parameters = function () {
    var result = getBasicParameters(10, 180, 5);
    var params = {
        'image': {
            name: "贴图",
            value: 'modellib/model/idc/images/inside_side.jpg',
            type: make.Default.PARAMETER_TYPE_IMAGE,
            propertyType: make.Default.PARAMETER_PROPERTY_TYPE_FIELD
        },
        'frontImage': {
            name: "贴图",
            value: 'modellib/model/idc/images/pdu2_wrap.jpg',
            type: make.Default.PARAMETER_TYPE_IMAGE,
            propertyType: make.Default.PARAMETER_PROPERTY_TYPE_FIELD
        }
    };
...
make.Default.register('twaver.idc.pdu2', function (json, callback) {
    var position = json.position || [0, 0, 0];
    var x = position[0],
        y = position[1],
        z = position[2];
    ...
    pdu.setClient('showShadow', false);
    ...
    category: '插座模型',
    type: 'pdu',
    modelDefaultParameters: getPdu2Parameters(),
})
...
make.Default.register('twaver.idc.oil_leaking_sign', function (json) {
    var radius = json.radius;
    image = getIdcImagePath(json.image);
    var sign = new mono.Billboard();
    sign.s({
...
make.Default.registerObj('twaver.idc.fpsModel', 'fpsModel', './model/idc/obj/fpsModel/', {}, false);
...
//红外线设备
make.Default.register('twaver.idc.infraredDevice', function (json, callback) {
    var from = json.from || [-500, 0],
        to = json.to || [500, 0],
    ...
    var path = new mono.Path();
    path.moveTo(from[0], from[1], 0);
    path.lineTo(to[0], to[1], 0);
    var cube = new mono.PathCube(path, radius * 2, height);
    cube.s({ 'm.opacity': 0, 'm.transparent': true });

    var angle = Math.atan2(to[1] - from[1], to[0] - from[0]),
        offset = 1.5,
        fromOffsetZ = offset * Math.sin(angle),
        fromOffsetX = offset * Math.cos(angle),
        toOffsetZ = offset * Math.sin(angle + Math.PI),
        toOffsetX = offset * Math.cos(angle + Math.PI);

    var pole = new mono.Cylinder(radius, radius, height);
    pole.s({
        'm.texture.image': getIdcImagePath('rack_inside.jpg'),
    ...
    pole.setPosition(from[0], height / 2, from[1]);
    // pole.setPosition(from[0], 0, from[1]);
    pole.setParent(cube);
    var glass = new mono.Cylinder(radius * 0.8, radius * 0.8, height * 0.75);
    var envmapArr = [];
    for (var i = 0; i < 6; i++) {
        envmapArr[i] = getIdcImagePath('room.jpg');
    }
    glass.s({
        'm.type': 'phong',
        'm.color': '#A9F5D0',
        'm.ambient': '#A9F5D0',
        'm.envmap.image': envmapArr,
    });
    glass.setParent(pole);
    glass.setPosition(fromOffsetX, 0, fromOffsetZ);

    var pole2 = pole.clone();
    pole2.setPosition(to[0], height / 2, to[1]);
    // pole2.setPosition(to[0], 0, to[1]);
    pole2.setParent(cube);
    var glass2 = glass.clone();
    glass2.setParent(pole2);
    glass2.setPosition(toOffsetX, 0, toOffsetZ);

    var gap = height * 0.75 / (number + 1);
    for (var i = 0; i < number; i++) {
        var emitPoint1 = new mono.Cube(1, 1, 1);
        emitPoint1.s({
            'm.color': color,
            'm.ambient': color,
        });
        emitPoint1.setPosition(from[0], height * 0.25 / 2 + gap * (i + 1), from[1]);
        emitPoint1.setParent(cube);
        var emitPoint2 = emitPoint1.clone();
        emitPoint2.setPosition(to[0], height * 0.25 / 2 + gap * (i + 1), to[1]);
        emitPoint2.setParent(cube);
        var link = new mono.Link(emitPoint1, emitPoint2);
        link.s({
            'm.color': color,
            'm.ambient': color,
            'm.type': 'phong',
        ...
        link.setClient('from', emitPoint1);
        link.setClient('to', emitPoint2);
        link.setParent(cube);
        link.setClient('animation', 'twinkle:0:0:easeNone');
    }
...
var getWrapModelDefaultParameters = function (model) {
    ...
    if (model.useDefaultIcon == false) { //useDefaultIcon 默认为true
        result.image = {
            name: "贴图",
...
make.Default.register("twaver.idc.device", function (json, callback) {
    json.name = json.type;
    if (make.Default._objObject[json.name]) {
        return make.Default.cloneObjElement(json, callback);
    } else {
        var height = json.height, width = json.width, depth = json.depth || 70;
        var deviceId = 'twaver.idc.device.' + height + 'u' + (json.isGltf ? '_gltf' : '');
        make.Default.load(deviceId, function (object) {
            // var objs = object.getDescendants();
            // objs = objs.map(function (o) {
                // var obox = object.getBoundingBoxWithChildren();
                // o.setScaleX(width / obox.size().x);
                // o.setScaleZ(depth / obox.size().z);
                // o.setPositionX(-obox.center().x * width / obox.size().x);
                // return o;
            // });
            var bbox = object.getBoundingBoxWithChildren();


                object.setScaleX(width / bbox.size().x);
                object.setScaleZ(depth / bbox.size().z);
                object.setPositionX(-bbox.center().x * width / bbox.size().x);

            var frontCube = new mono.Cube(width + 5, 4.45 * height, 1),
                backCube = new mono.Cube(width, 4.45 * height, 0.1);
            ...
            var objs = object.getDescendants();
            objs.unshift(object);
            make.Default._objObject[json.name] = mono.Utils.mergeElements(objs);
...
make.Default.register('twaver.idc.mulFloor', function (json, callback) {
    var floors = json.floors || [];
    var parent = new mono.Cube(1, 1, 1);

    if (mono.Utils.isArray(floors) && floors.length > 0) {
        for (var i = 0; i < floors.length; i++) {
            var position = floors[i].position || [0, 0, 0];
            var floor = make.Default.load({ id: "twaver.combo", data: floors[i].data });
            floor.p(position[0], position[1], position[2]);
            floor.setParent(parent);
            console.log(parent.getChildren().size());
        }
    }
    /*for(var i = 0; i < floors.length; i++){
        var floor = make.Default.load({id: "twaver.combo",data: floors[i].data});
    ...
    icon: 'model/idc/icons/floor.png',
    category: "floor",
    type: "mono.Element",
    name: "多地板组合模型",
    description: "多地板组合模型",
...
for (var i = 1; i <= 8; i++) {
    make.Default.registerObj({
        id: 'twaver.idc.device.' + i + 'u',
        name: 'device.' + i + 'u',
        path: make.Default.path + 'model/idc/obj/' + i + 'u/'
    });
    make.Default.registerObj({
        id: 'twaver.idc.device.' + i + 'u_gltf',
        name: 'device.' + i + 'u_gltf',
        path: make.Default.path + 'model/idc/obj/' + i + 'u_gltf/',
        isGltf: true,
        flipY: true
    });
}
...
var panelDef = {
    ...
    "equipment.ibm.ibm_x3750_m4": { label: "equipment.ibm.ibm_x3750_m4", width: 450.03, height: 88.91, size: 2 },
    ...
    "equipment.ibm.ibm_power570": { label: "equipment.ibm.ibm_power570", width: 450.00, height: 180, size: 4 },
    ...
    "equipment.ibm.ibm_v5000": { label: "equipment.ibm.ibm_v5000", width: 450.00, height: 90, size: 2 },
    ...
    "TOPSEC_NGFW4000": { label: "TOPSEC_NGFW4000", width: 500.03, height: 89.00, size: 2 },
    "TOPSEC_NGFW4000_back": { label: "TOPSEC_NGFW4000_back", width: 450.03, height: 89.00, size: 2 },
    "TOPSEC_TopFilter8000": { label: "TOPSEC_TopFilter8000", width: 500.03, height: 89.00, size: 2 },
    ...
    "h3c.s5120_series": { label: "H3C_S5120_Series", width: 450.03, height: 44.50, size: 1 },
    ...
    "huawei.quidway_s5700series": { label: "huawei.quidway_s5700series", width: 500.03, height: 44.50, size: 1 },
    "fiberhome.022070": { label: "fiberhome.022070", width: 500.03, height: 445.03, size: 10 },
    "fiberhome.022070_back": { label: "fiberhome.022070_back", width: 450.03, height: 445.03, size: 10 },
    "h3c.s5120_series": { label: "h3c_s5120_series", width: 500.03, height: 44.50, size: 1 },
    "h3c.s5120_series_back": { label: "h3c_s5120_series_back", width: 450.03, height: 44.50, size: 1 },
    "equipment.accelink.OLP_EDFA": { label: "equipment.accelink.OLP_EDFA", width: 500.03, height: 44.50, size: 1 },
    "equipment.accelink.OLP_EDFA_back": { label: "equipment.accelink.OLP_EDFA_back", width: 450.03, height: 44.50, size: 1 },
    "equipment.bland.RackSwitch_G8000": { label: "equipment.bland.RackSwitch_G8000", width: 500.03, height: 44.50, size: 1 },
    "equipment.bland.RackSwitch_G8000_back": { label: "equipment.bland.RackSwitch_G8000_back", width: 450.03, height: 44.50, size: 1 },
    "equipment.dbappsecurity.071622": { label: "equipment.dbappsecurity.071622", width: 500.03, height: 44.50, size: 1 },
    "equipment.dahua.099051": { label: "equipment.dahua.099051", width: 500.03, height: 44.50, size: 1 },
    "equipment.dahua.099051_back": { label: "equipment.dahua.099051_back", width: 450.03, height: 44.50, size: 1 },
    "equipment.dbappsecurity.065519": { label: "equipment.dbappsecurity.065519", width: 500.03, height: 89.00, size: 2 },
    "equipment.dbappsecurity.092694": { label: "equipment.dbappsecurity.092694", width: 500.03, height: 44.50, size: 1 },
    "equipment.dell.AMP01": { label: "equipment.dell.AMP01", width: 500.03, height: 133.51, size: 3 },
    "equipment.dell.EqualLogic_PS6510": { label: "equipment.dell.EqualLogic_PS6510", width: 500.03, height: 178.01, size: 4 },
    "equipment.dell.PowerEdge_C6100": { label: "equipment.dell.PowerEdge_C6100", width: 500.03, height: 89.00, size: 2 },
    "equipment.dell.PowerEdge_R730": { label: "equipment.dell.PowerEdge_R730", width: 500.03, height: 89.00, size: 2 },
    "equipment.dell.PowerEdge_R730VD": { label: "equipment.dell.PowerEdge_R730VD", width: 500.03, height: 89.00, size: 2 },
    "equipment.dell.PowerEdge_R810": { label: "equipment.dell.PowerEdge_R810", width: 500.03, height: 89.00, size: 2 },
    "equipment.dell.PowerEdge_R930": { label: "equipment.dell.PowerEdge_R930", width: 500.03, height: 178.01, size: 4 },
    "equipment.emc.DS_5100B": { label: "equipment.emc.DS_5100B", width: 500.03, height: 44.50, size: 1 },
    "equipment.dell.PowerEdge_R910": { label: "equipment.dell.PowerEdge_R910", width: 500.03, height: 178.01, size: 4 },
    "equipment.fablesoft.056488": { label: "equipment.fablesoft.056488", width: 500.03, height: 89.00, size: 2 },
    "equipment.fiberhome.009936_back": { label: "equipment.fiberhome.009936_back", width: 450.03, height: 89.00, size: 2 },
    "equipment.fiberhome.009936": { label: "equipment.fiberhome.009936", width: 500.03, height: 89.00, size: 2 },
    "equipment.fiberhome.058025": { label: "equipment.fiberhome.058025", width: 500.03, height: 89.00, size: 2 },
    "equipment.fiberhome.058025_back": { label: "equipment.fiberhome.058025_back", width: 450.03, height: 89.00, size: 2 },
    "equipment.fiberhome.063720": { label: "equipment.fiberhome.063720", width: 500.03, height: 267.02, size: 6 },
    "equipment.fiberhome.069184": { label: "equipment.fiberhome.069184", width: 500.03, height: 356.02, size: 8 },
    "equipment.fiberhome.089744": { label: "equipment.fiberhome.089744", width: 500.03, height: 400.53, size: 9 },
    "equipment.fiberhome.FS5512": { label: "equipment.fiberhome.FS5512", width: 500.03, height: 445.03, size: 10 },
    "equipment.fiberhome.H03_512G": { label: "equipment.fiberhome.H03_512G", width: 500.03, height: 845.55, size: 19 },
    "equipment.fiberhome.M100E": { label: "equipment.fiberhome.M100E", width: 500.03, height: 445.03, size: 10 },
    "equipment.fiberhome.H03_512G_back": { label: "equipment.fiberhome.H03_512G_back", width: 450.03, height: 845.55, size: 19 },
    "equipment.fiberhome.S03_NEBULA_FP_500M": { label: "equipment.fiberhome.S03_NEBULA_FP_500M", width: 500.03, height: 222.51, size: 5 },
    "equipment.fiberhome.M100E_back": { label: "equipment.fiberhome.M100E_back", width: 500.03, height: 445.03, size: 10 },
    "equipment.h3c.5120_Series": { label: "equipment.h3c.5120_Series", width: 500.03, height: 44.50, size: 1 },
    "equipment.fiberhome.S03_NEBULA_FP_500M_back": { label: "equipment.fiberhome.S03_NEBULA_FP_500M_back", width: 450.03, height: 222.51, size: 5 },
    "equipment.hikvision.051274": { label: "equipment.hikvision.051274", width: 500.03, height: 89.00, size: 2 },
    "equipment.hp.Proliant_DL360_G5": { label: "equipment.hp.Proliant_DL360_G5", width: 500.03, height: 44.50, size: 1 },
    "equipment.hp.Proliant_DL360_G5_back": { label: "equipment.hp.Proliant_DL360_G5_back", width: 450.03, height: 44.50, size: 1 },
    "equipment.fiberhome.Support_6GB": { label: "equipment.fiberhome.Support_6GB", width: 500.03, height: 89.00, size: 2 },
    "equipment.hp.Proliant_DL380_G5": { label: "equipment.hp.Proliant_DL380_G5", width: 500.03, height: 89.00, size: 2 },
    "equipment.hp.Proliant_DL380_G5_back": { label: "equipment.hp.Proliant_DL380_G5_back", width: 450.03, height: 89.00, size: 2 },
    "equipment.hp.Proliant_SL2x170ZG6": { label: "equipment.hp.Proliant_SL2x170ZG6", width: 500.03, height: 89.00, size: 2 },
    "equipment.hp.StoageWorks_HSV300_A": { label: "equipment.hp.StoageWorks_HSV300_A", width: 500.03, height: 89.00, size: 2 },
    "equipment.hp.StoageWorks": { label: "equipment.hp.StoageWorks", width: 500.03, height: 133.51, size: 3 },
    "equipment.hp.StoageWorks_HSV300_A_back": { label: "equipment.hp.StoageWorks_HSV300_A_back", width: 450.03, height: 89.00, size: 2 },
    "equipment.huawei.Quidway_S5600_Series": { label: "equipment.huawei.Quidway_S5600_Series", width: 500.03, height: 44.50, size: 1 },
    "equipment.huawei.033466": { label: "equipment.huawei.033466", width: 500.03, height: 178.01, size: 4 },
    "equipment.huawei.Ocean_Stor_S2600T": { label: "equipment.huawei.Ocean_Stor_S2600T", width: 500.03, height: 89.00, size: 2 },
    "equipment.huawei.S1700_24GR": { label: "equipment.huawei.S1700_24GR", width: 500.03, height: 44.50, size: 1 },
    "equipment.huawei.S9706": { label: "equipment.huawei.S9706", width: 500.03, height: 445.03, size: 10 },
    "equipment.inspur.18000": { label: "equipment.inspur.18000", width: 500.03, height: 356.02, size: 8 },
    "equipment.Jetstor.DS3800": { label: "equipment.Jetstor.DS3800", width: 500.03, height: 178.01, size: 4 },
    "equipment.inspur.NF5270M4": { label: "equipment.inspur.NF5270M4", width: 500.03, height: 89.00, size: 2 },
    "equipment.netpower.NPFW_1000": { label: "equipment.netpower.NPFW_1000", width: 500.03, height: 89.00, size: 2 },
    "equipment.huawei.S9706_back": { label: "equipment.huawei.S9706_back", width: 450.03, height: 445.03, size: 10 },
    "equipment.Jetstor.DS3800_back": { label: "equipment.Jetstor.DS3800_back", width: 450.03, height: 178.01, size: 4 },
    "equipment.nsfocus.RSAS_S_Series": { label: "equipment.nsfocus.RSAS_S_Series", width: 500.03, height: 44.50, size: 1 },
    "equipment.netpower.NPFW_1000_back": { label: "equipment.netpower.NPFW_1000_back", width: 450.03, height: 89.00, size: 2 },
    "equipment.run.020030": { label: "equipment.run.020030", width: 450.03, height: 89.00, size: 2 },
    "equipment.nsfocus.RSAS_S_Series_back": { label: "equipment.nsfocus.RSAS_S_Series_back", width: 450.03, height: 44.50, size: 1 },
    "equipment.run.031512": { label: "equipment.run.031512", width: 450.03, height: 89.00, size: 2 },
    "equipment.run.054007": { label: "equipment.run.054007", width: 450.03, height: 178.01, size: 4 },
    "equipment.run.054007_back": { label: "equipment.run.054007_back", width: 450.03, height: 178.01, size: 4 },
    "equipment.run.073445": { label: "equipment.run.073445", width: 450.03, height: 133.51, size: 3 },
    "equipment.run.073445_back": { label: "equipment.run.073445_back", width: 450.03, height: 133.51, size: 3 },
    "equipment.run.090729": { label: "equipment.run.090729", width: 450.03, height: 178.01, size: 4 },
    "equipment.run.BP": { label: "equipment.run.BP", width: 450.03, height: 89.00, size: 2 },
    "equipment.run.035143": { label: "equipment.run.035143", width: 500.03, height: 623.04, size: 14 },
    "equipment.run.NPA8000_1": { label: "equipment.run.NPA8000_1", width: 450.03, height: 89.00, size: 2 },
    "equipment.run.H22M_03": { label: "equipment.run.H22M_03", width: 450.03, height: 89.00, size: 2 },
    "equipment.run.H22M_03_back": { label: "equipment.run.H22M_03_back", width: 450.03, height: 89.00, size: 2 },
    "equipment.run.NPA8000_2": { label: "equipment.run.NPA8000_2", width: 450.03, height: 89.00, size: 2 },
    "equipment.run.NSE1148_EA": { label: "equipment.run.NSE1148_EA", width: 450.03, height: 44.50, size: 1 },
    "equipment.run.ONEWAY1000_A": { label: "equipment.run.ONEWAY1000_A", width: 450.03, height: 89.00, size: 2 },
    "equipment.serva.005894": { label: "equipment.serva.005894", width: 500.03, height: 178.01, size: 4 },
    "equipment.serva.014697": { label: "equipment.serva.014697", width: 450.03, height: 178.01, size: 4 },
    "equipment.serva.014697_back": { label: "equipment.serva.014697_back", width: 450.03, height: 178.01, size: 4 },
    "equipment.serva.034961": { label: "equipment.serva.034961", width: 450.03, height: 44.50, size: 1 },
    "equipment.serva.034961_back": { label: "equipment.serva.034961_back", width: 450.03, height: 44.50, size: 1 },
    "equipment.serva.020776": { label: "equipment.serva.020776", width: 500.03, height: 44.50, size: 1 },
    "equipment.serva.054861": { label: "equipment.serva.054861", width: 450.03, height: 44.50, size: 1 },
    "equipment.serva.054861_back": { label: "equipment.serva.054861_back", width: 450.03, height: 44.50, size: 1 },
    "equipment.serva.076575": { label: "equipment.serva.076575", width: 500.03, height: 44.50, size: 1 },
    "equipment.serva.055992": { label: "equipment.serva.055992", width: 500.03, height: 44.50, size: 1 },
    "equipment.serva.083159": { label: "equipment.serva.083159", width: 500.03, height: 44.50, size: 1 },
    "equipment.serva.077978": { label: "equipment.serva.077978", width: 500.03, height: 44.50, size: 1 },
    "equipment.serva.092567": { label: "equipment.serva.092567", width: 500.03, height: 44.50, size: 1 },
    "equipment.serva.GA_TBB_1000B": { label: "equipment.serva.GA_TBB_1000B", width: 500.03, height: 89.00, size: 2 },
    "equipment.surfilter.SURF_NGSA_S2100": { label: "equipment.surfilter.SURF_NGSA_S2100", width: 500.03, height: 44.50, size: 1 },
    "equipment.topsec.TOPIDP2000": { label: "equipment.topsec.TOPIDP2000", width: 500.03, height: 44.50, size: 1 },
    "equipment.tplink.TL_SF1016S": { label: "equipment.tplink.TL_SF1016S", width: 500.03, height: 44.50, size: 1 },
    "equipment.dell.DELL_R530_back": { label: "equipment.dell.DELL_R530_back", width: 450.03, height: 89.00, size: 2 },
    "equipment.dell.PowerEdge_R730_back": { label: "equipment.dell.PowerEdge_R730_back", width: 450.03, height: 89.00, size: 2 },
    "equipment.h3c.56800_4C_back": { label: "equipment.h3c.56800_4C_back", width: 450.03, height: 89.00, size: 2 },
    "equipment.h3c.56800_4C": { label: "equipment.h3c.56800_4C", width: 450.03, height: 89.00, size: 2 },
    "equipment.h3c.s10508_back": { label: "equipment.h3c.s10508_back", width: 450.03, height: 667.5, size: 15 },
    "equipment.h3c.s10508": { label: "equipment.h3c.s10508", width: 450.03, height: 667.5, size: 15 },
    "equipment.h3c.s1850_28p": { label: "equipment.h3c.s1850_28p", width: 450.03, height: 44.50, size: 1 },
    "equipment.huawei.DAE22435U4_back": { label: "equipment.huawei.DAE22435U4_back", width: 450.03, height: 177.81, size: 4 },
    "equipment.huawei.oceanstor_sns2124": { label: "equipment.huawei.oceanstor_sns2124", width: 450.03, height: 44.50, size: 1 },
    "equipment.huawei.rh2288h_v23_3_back": { label: "equipment.huawei.rh2288h_v23_3_back", width: 450.03, height: 89.00, size: 2 },
    "equipment.huawei.SPE33C0225_back": { label: "equipment.huawei.SPE33C0225_back", width: 450.03, height: 89.00, size: 2 },
    "equipment.inspur.AS300N_back": { label: "equipment.inspur.AS300N_back", width: 450.03, height: 89.00, size: 2 },
    "equipment.inspur.NF8460M4_back": { label: "equipment.inspur.NF8460M4_back", width: 450.03, height: 177.81, size: 4 },
    "equipment.inspur.TS860G3_back": { label: "equipment.inspur.TS860G3_back", width: 450.03, height: 400.5, size: 9 },
    "equipment.inspur.TS860G3": { label: "equipment.inspur.TS860G3", width: 450.03, height: 400.5, size: 9 },
    "equipment.leadsec.sis_3000": { label: "equipment.leadsec.sis_3000", width: 450.03, height: 89.00, size: 2 },
    "equipment.polycom.1257_back": { label: "equipment.polycom.1257_back", width: 450.03, height: 89.00, size: 2 },
    "equipment.sangfor.1246": { label: "equipment.sangfor.1246", width: 450.03, height: 89.00, size: 2 },
    "equipment.serva.1248_back": { label: "equipment.serva.1248_back", width: 450.03, height: 89.00, size: 2 },
    "equipment.serva.1249_back": { label: "equipment.serva.1249_back", width: 450.03, height: 133.5, size: 3 },
    "equipment.serva.adpm2420_80_back": { label: "equipment.serva.adpm2420_80_back", width: 450.03, height: 178, size: 4 },
    "equipment.topsec.1267": { label: "equipment.topsec.1267", width: 450.03, height: 44.50, size: 1 },
    "equipment.topsec.ngfw4000-uf2": { label: "equipment.topsec.ngfw4000-uf2", width: 450.03, height: 89.00, size: 2 },
    "equipment.topsec.ta-71220": { label: "equipment.topsec.ta-71220", width: 450.03, height: 89.00, size: 2 },
    "equipment.uniview.vx_1660_back": { label: "equipment.uniview.vx_1660_back", width: 450.03, height: 178, size: 4 },
    "equipment.unv.1252_back": { label: "equipment.unv.1252_back", width: 450.03, height: 133.5, size: 3 },
    "equipment.unv.1253_back": { label: "equipment.unv.1253_back", width: 450.03, height: 178, size: 4 },
    "equipment.videocon.1254": { label: "equipment.videocon.1254", width: 450.03, height: 89.00, size: 2 },
    "equipment.xbrother.1255_back": { label: "equipment.xbrother.1255_back", width: 450.03, height: 89.00, size: 2 },
    "equipment.amp.1327_back": { label: "equipment.amp.1327_back", width: 450.03, height: 89.00, size: 2 },
    "equipment.amp.1327": { label: "equipment.amp.1327", width: 450.03, height: 89.00, size: 2 },
    "equipment.sun.sparc_enterprise_t5240": { label: "equipment.sun.sparc_enterprise_t5240", width: 450.03, height: 89.00, size: 2 },
    "equipment.sun.sparc_enterprise_t5240_back": { label: "equipment.sun.sparc_enterprise_t5240_back", width: 450.03, height: 89.00, size: 2 },
    "equipment.sun.stordege3300": { label: "equipment.sun.stordege3300", width: 450.03, height: 89.00, size: 2 },
    "equipment.sun.stordege3300_back": { label: "equipment.sun.stordege3300_back", width: 450.03, height: 89.00, size: 2 },
    "equipment.macrosan.ms3000": { label: "equipment.macrosan.ms3000", width: 450.03, height: 89.00, size: 4 },
    "equipment.macrosan.ms3000_back": { label: "equipment.macrosan.ms3000_back", width: 450.03, height: 89.00, size: 4 },
    "equipment.dell.DELL_R530": { label: "equipment.dell.DELL_R530", width: 450.03, height: 89.00, size: 2 },
    "equipment.dell.PowerEdge_R730": { label: "equipment.dell.PowerEdge_R730", width: 450.03, height: 89.00, size: 2 },
    "equipment.huawei.DAE22435U4": { label: "equipment.huawei.DAE22435U4", width: 450.03, height: 177.81, size: 4 },
    "equipment.huawei.rh2288h_v23_3": { label: "equipment.huawei.rh2288h_v23_3", width: 450.03, height: 89.00, size: 2 },
    "equipment.huawei.SPE33C0225": { label: "equipment.huawei.SPE33C0225", width: 450.03, height: 89.00, size: 2 },
    "equipment.inspur.AS300N": { label: "equipment.inspur.AS300N", width: 450.03, height: 89.00, size: 2 },
    "equipment.inspur.NF8460M4": { label: "equipment.inspur.NF8460M4", width: 450.03, height: 177.81, size: 4 },
    "equipment.polycom.1257": { label: "equipment.polycom.1257", width: 450.03, height: 89.00, size: 2 },
    "equipment.serva.1248": { label: "equipment.serva.1248", width: 450.03, height: 89.00, size: 2 },
    "equipment.serva.1249": { label: "equipment.serva.1249", width: 450.03, height: 133.5, size: 3 },
    "equipment.serva.adpm2420_80": { label: "equipment.serva.adpm2420_80", width: 450.03, height: 178, size: 4 },
    "equipment.uniview.vx_1660": { label: "equipment.uniview.vx_1660", width: 450.03, height: 178, size: 4 },
    "equipment.unv.1252": { label: "equipment.unv.1252", width: 450.03, height: 133.5, size: 3 },
    "equipment.unv.1253": { label: "equipment.unv.1253", width: 450.03, height: 178, size: 4 },
    "equipment.xbrother.1255": { label: "equipment.xbrother.1255", width: 450.03, height: 89.00, size: 2 },
    "equipment.huawei.etp48300_a2": { label: "equipment.huawei.etp48300_a2", width: 450.03, height: 89.12, size: 2 },
    "equipment.huawei.cloudengine_12800": { label: "equipment.huawei.cloudengine_12800", width: 450.03, height: 445.02, size: 10 },
    "equipment.huawei.nip6000": { label: "equipment.huawei.nip6000", width: 500.02, height: 133.50, size: 3 },
    "equipment.huawei.hw_s5720-56c-ei-48s-ac": { label: "equipment.huawei.hw_s5720-56c-ei-48s-ac", width: 450.03, height: 44.49, size: 1 },
    "equipment.huawei.nip6000_back": { label: "equipment.huawei.nip6000_back", width: 450.03, height: 133.50, size: 3 },
    "equipment.huawei.e9000": { label: "equipment.huawei.e9000", width: 450.03, height: 534.03, size: 12 },
    "equipment.huawei.e9000_back": { label: "equipment.huawei.e9000_back", width: 450.03, height: 534.03, size: 12 },
    "equipment.huawei.oceanstor_5500_v3_2": { label: "equipment.huawei.oceanstor_5500_v3_2", width: 500.02, height: 89.01, size: 2 },
    "equipment.huawei.oceanstor_5500_v3_2_back": { label: "equipment.huawei.oceanstor_5500_v3_2_back", width: 450.03, height: 89.01, size: 2 },
    "equipment.huawei.s6720_30c_ei_24s": { label: "equipment.huawei.s6720_30c_ei_24s", width: 500.02, height: 44.49, size: 1 },
    "equipment.huawei.s6720_30c_ei_24s_back": { label: "equipment.huawei.s6720_30c_ei_24s_back", width: 450.03, height: 44.49, size: 1 },
    "equipment.huawei.s6720_30c_ei_24s-2": { label: "equipment.huawei.s6720_30c_ei_24s", width: 500.02, height: 44.49, size: 1 },
    "equipment.huawei.s6720_30c_ei_24s-2_back": { label: "equipment.huawei.s6720_30c_ei_24s_back", width: 450.03, height: 44.49, size: 1 },
    "equipment.huawei.s12704": { label: "equipment.huawei.s12704", width: 500.02, height: 489.55, size: 10 },
    "equipment.huawei.s12704-2": { label: "equipment.huawei.s12704", width: 500.02, height: 489.55, size: 10 },
    "equipment.huawei.cloudengine_2800_2": { label: "equipment.huawei.cloudengine_2800_2", width: 450.03, height: 801.07, size: 17 },
    "equipment.huawei.cloudengine_2800_2-2": { label: "equipment.huawei.cloudengine_2800_2", width: 450.03, height: 801.07, size: 17 },
    "equipment.huawei.secospace_usg9000_series_back": { label: "equipment.huawei.secospace_usg9000_series_back", width: 450.03, height: 222.51, size: 5 },
    "equipment.mediamatrix.cab_8n": { label: "equipment.mediamatrix.cab_8n", width: 500.02, height: 44.49, size: 1 },
    "equipment.mediamatrix.cab_8n_back": { label: "equipment.mediamatrix.cab_8n_back", width: 450.03, height: 44.49, size: 1 },
    "equipment.huawei.secospace_usg9000_series": { label: "equipment.huawei.secospace_usg9000_series", width: 450.03, height: 222.51, size: 5 },
    "equipment.mediamatrix.nion_n3_back": { label: "equipment.mediamatrix.nion_n3_back", width: 450.03, height: 89.01, size: 2 },
    "equipment.mediamatrix.nion_n3": { label: "equipment.mediamatrix.nion_n3", width: 450.03, height: 89.01, size: 2 },
    "equipment.radware.alteon_6420": { label: "equipment.radware.alteon_6420", width: 500.02, height: 89.01, size: 2 },
    "equipment.rosenberger.2206_back": { label: "equipment.rosenberger.2206_back", width: 450.03, height: 44.49, size: 1 },
    "equipment.rosenberger.2206": { label: "equipment.rosenberger.2206", width: 500.02, height: 44.49, size: 1 },
    "equipment.radware.alteon_6420_back": { label: "equipment.radware.alteon_6420_back", width: 450.03, height: 89.01, size: 2 },
    "equipment.rosenberger.2211": { label: "equipment.rosenberger.2211", width: 500.02, height: 44.49, size: 1 },
    "equipment.rosenberger.2210": { label: "equipment.rosenberger.2210", width: 500.02, height: 44.49, size: 1 },
    "equipment.rosenberger.hdcs_back": { label: "equipment.rosenberger.hdcs_back", width: 450.03, height: 133.50, size: 3 },
    "equipment.rosenberger.hdcs": { label: "equipment.rosenberger.hdcs", width: 500.02, height: 133.50, size: 3 },
    "equipment.serva.2212": { label: "equipment.serva.2212", width: 500.02, height: 178.02, size: 4 },
    "equipment.serva.2212_back": { label: "equipment.serva.2212_back", width: 450.03, height: 178.02, size: 4 },
    "equipment.sun.sparc_s7_2l": { label: "equipment.sun.sparc_s7_2l", width: 450.03, height: 89.01, size: 2 },
    "equipment.huawei.cloudengine_12800_back": { label: "equipment.huawei.cloudengine_12800_back", width: 450.03, height: 445.02, size: 10 },
    "equipment.huawei.ecns210": { label: "equipment.huawei.ecns210", width: 450.03, height: 578.52, size: 14 },
    "equipment.huawei.etp48300_a2_back": { label: "equipment.huawei.etp48300_a2_back", width: 450.03, height: 89.01, size: 2 },
    "equipment.huawei.oceanstor_sns2224": { label: "equipment.huawei.oceanstor_sns2224", width: 450.03, height: 44.49, size: 1 },
    "equipment.huawei.ecns210_back": { label: "equipment.huawei.ecns210_back", width: 450.03, height: 578.52, size: 13 },
    "equipment.huawei.oceanstor_sns2224_back": { label: "equipment.huawei.oceanstor_sns2224_back", width: 450.03, height: 44.49, size: 1 },
    "equipment.huawei.cloudengine_2800_2_back": { label: "equipment.huawei.cloudengine_2800_2_back", width: 450.03, height: 801.07, size: 17 },
    "equipment.huawei.cloudengine_2800_2-2_back": { label: "equipment.huawei.cloudengine_2800_2_back", width: 450.03, height: 801.07, size: 17 },
    "equipment.huawei.oceanstor_sns3096_back": { label: "equipment.huawei.oceanstor_sns3096_back", width: 450.03, height: 89.08, size: 2 },
    "equipment.huawei.oceanstor_sns3096": { label: "equipment.huawei.oceanstor_sns3096", width: 450.03, height: 89.01, size: 2 },
    "equipment.huawei.ce6855_48s6q_hi-2_back": { label: "equipment.huawei.ce6855_48s6q_hi_back", width: 450.03, height: 44.49, size: 1 },
    "equipment.huawei.ce6855_48s6q_hi-2_back": { label: "equipment.huawei.ce6855_48s6q_hi_back", width: 450.03, height: 44.49, size: 1 },
    "equipment.advantech.dtst_8U": { label: "equipment.advantech.dtst_8U", width: 450.03, height: 356.01, size: 8 },
    "equipment.advantech.dtst_8U_back": { label: "equipment.advantech.dtst_8U_back", width: 455.04, height: 356.01, size: 8 },
    "equipment.huawei.PowerEdge_R730": { label: "equipment.huawei.PowerEdge_R730", width: 450.03, height: 89.01, size: 2 },
    "equipment.huawei.rh2288h_v3": { label: "equipment.huawei.rh2288h_v3", width: 450.03, height: 89.01, size: 2 },
    "equipment.huawei.ce6855_48s6q_hi": { label: "equipment.huawei.ce6855_48s6q_hi", width: 450.03, height: 44.49, size: 1 },
    "equipment.huawei.ce6855_48s6q_hi-2": { label: "equipment.huawei.ce6855_48s6q_hi-2", width: 450.03, height: 44.49, size: 1 },
    "equipment.huawei.rh2288h_v3_back": { label: "equipment.huawei.rh2288h_v3_back", width: 450.03, height: 89.01, size: 2 },
    "equipment.huawei.PowerEdge_R730_back": { label: "equipment.huawei.PowerEdge_R730_back", width: 450.03, height: 89.01, size: 2 },
    "equipment.huawei.rh5885_v3": { label: "equipment.huawei.rh5885_v3", width: 450.03, height: 178.02, size: 4 },
    "equipment.huawei.rh5885_v3_back": { label: "equipment.huawei.rh5885_v3_back", width: 450.03, height: 178.02, size: 4 },
    "equipment.huawei.secospace_usg6000_back": { label: "equipment.huawei.secospace_usg6000_back", width: 450.03, height: 44.49, size: 1 },
    "equipment.huawei.secospace_usg6000": { label: "equipment.huawei.secospace_usg6000", width: 450.03, height: 44.49, size: 1 },
    "equipment.emerson.avocent_lra185": { label: "equipment.emerson.avocent_lra185", width: 451.09, height: 45.51, size: 1 },
    "equipment.mellanox.sx6036_back": { label: "equipment.mellanox.sx6036_back", width: 450.03, height: 44.52, size: 1 },
    "equipment.mellanox.sx6036": { label: "equipment.mellanox.sx6036", width: 457.76, height: 34.96, size: 1 },
    "equipment.huawei.2443_back": { label: "equipment.huawei.2443_back", width: 450.03, height: 178.02, size: 4 },
    "equipment.huawei.2443": { label: "equipment.huawei.2443", width: 450.28, height: 178.09, size: 4 },
    "equipment.serva.2447": { label: "equipment.serva.2447", width: 450.03, height: 44.49, size: 1 },
    "equipment.serva.2446": { label: "equipment.serva.2446", width: 450.67, height: 89.01, size: 2 },
    "equipment.serva.2446_back": { label: "equipment.serva.2446_back", width: 450.60, height: 89.36, size: 2 },
    "equipment.serva.eq_10": { label: "equipment.serva.eq_10", width: 500.02, height: 445.02, size: 10 },
    "equipment.serva.eq_11": { label: "equipment.serva.eq_11", width: 500.02, height: 489.55, size: 11 },
    "equipment.serva.eq_11_back": { label: "equipment.serva.eq_11_back.svg", width: 451.02, height: 490.53, size: 11 },
    "equipment.serva.eq_12": { label: "equipment.serva.eq_12", width: 500.02, height: 534.03, size: 12 },
    "equipment.serva.eq_10_back": { label: "equipment.serva.eq_10_back", width: 451.02, height: 446.05, size: 10 },
    "equipment.serva.eq_12_back": { label: "equipment.serva.eq_12_back", width: 451.02, height: 535.02, size: 12 },
    "equipment.serva.eq_13_back": { label: "equipment.serva.eq_13_back", width: 451.02, height: 579.54, size: 13 },
    "equipment.serva.eq_13": { label: "equipment.serva.eq_13", width: 500.02, height: 578.52, size: 13 },
    "equipment.serva.eq_14": { label: "equipment.serva.eq_14", width: 500.02, height: 623.04, size: 14 },
    "equipment.serva.eq_14_back": { label: "equipment.serva.eq_14_back", width: 451.02, height: 624.03, size: 14 },
    "equipment.serva.eq_15": { label: "equipment.serva.eq_15", width: 500.02, height: 667.53, size: 15 },
    "equipment.serva.eq_16": { label: "equipment.serva.eq_16", width: 500.02, height: 712.06, size: 16 },
    "equipment.serva.eq_15_back": { label: "equipment.serva.eq_15_back", width: 451.02, height: 668.56, size: 15 },
    "equipment.serva.eq_16_back": { label: "equipment.serva.eq_16_back", width: 451.02, height: 713.04, size: 16 },
    "equipment.serva.eq_17": { label: "equipment.serva.eq_17", width: 500.02, height: 756.54, size: 17 },
    "equipment.serva.eq_17_back": { label: "equipment.serva.eq_17_back", width: 451.02, height: 757.53, size: 17 },
    "equipment.serva.eq_18_back": { label: "equipment.serva.eq_18_back", width: 451.02, height: 802.06, size: 18 },
    "equipment.serva.eq_18": { label: "equipment.serva.eq_18", width: 500.02, height: 801.07, size: 18 },
    "equipment.serva.eq_19": { label: "equipment.serva.eq_19", width: 500.02, height: 845.56, size: 19 },
    "equipment.serva.eq_19_back": { label: "equipment.serva.eq_19_back", width: 451.02, height: 846.97, size: 19 },
    "equipment.serva.eq_20": { label: "equipment.serva.eq_20", width: 500.02, height: 890.04, size: 20 },
    "equipment.serva.eq_20_back": { label: "equipment.serva.eq_20_back", width: 451.02, height: 891.07, size: 20 },
    "equipment.serva.eq_21": { label: "equipment.serva.eq_21", width: 500.02, height: 934.57, size: 21 },
    "equipment.serva.eq_21_back": { label: "equipment.serva.eq_21_back", width: 451.02, height: 935.56, size: 21 },
    "equipment.serva.eq_22": { label: "equipment.serva.eq_22", width: 500.02, height: 979.06, size: 22 },
    "equipment.serva.eq_22_back": { label: "equipment.serva.eq_22_back", width: 13.94, height: 2.47, size: 22 },
    "equipment.serva.eq_23": { label: "equipment.serva.eq_23", width: 500.02, height: 1023.58, size: 23 },
    "equipment.serva.eq_23_back": { label: "equipment.serva.eq_23_back", width: 451.02, height: 1024.57, size: 23 },
    "equipment.serva.eq_24": { label: "equipment.serva.eq_24", width: 500.02, height: 1068.07, size: 24 },
    "equipment.serva.eq_24_back": { label: "equipment.serva.eq_24_back", width: 451.02, height: 1069.05, size: 24 },
    "equipment.serva.eq_25": { label: "equipment.serva.eq_25", width: 500.02, height: 1112.55, size: 25 },
    "equipment.serva.eq_26": { label: "equipment.serva.eq_26", width: 500.02, height: 1157.08, size: 26 },
    "equipment.serva.eq_25_back": { label: "equipment.serva.eq_25_back", width: 451.02, height: 1113.58, size: 25 },
    "equipment.serva.eq_26_back": { label: "equipment.serva.eq_26_back", width: 451.02, height: 1158.07, size: 26 },
    "equipment.serva.eq_27": { label: "equipment.serva.eq_27", width: 500.02, height: 1201.57, size: 27 },
    "equipment.serva.eq_27_back": { label: "equipment.serva.eq_27_back", width: 451.02, height: 1202.73, size: 27 },
    "equipment.serva.eq_28": { label: "equipment.serva.eq_28", width: 500.02, height: 1246.09, size: 28 },
    "equipment.serva.eq_28_back": { label: "equipment.serva.eq_28_back", width: 451.02, height: 1247.08, size: 28 },
    "equipment.serva.eq_29": { label: "equipment.serva.eq_29", width: 500.02, height: 1290.58, size: 29 },
    "equipment.serva.eq_29_back": { label: "equipment.serva.eq_29_back", width: 451.02, height: 1291.60, size: 29 },
    "equipment.serva.eq_1": { label: "equipment.serva.eq_1", width: 500.02, height: 44.4, size: 1 },
    "equipment.serva.eq_2": { label: "equipment.serva.eq_2", width: 500.02, height: 89.01, size: 2 },
    "equipment.serva.eq_30": { label: "equipment.serva.eq_30", width: 500.02, height: 1335.10, size: 30 },
    "equipment.serva.eq_30_back": { label: "equipment.serva.eq_30_back", width: 451.02, height: 1337.01, size: 30 },
    "equipment.serva.eq_31": { label: "equipment.serva.eq_31", width: 500.02, height: 1379.59, size: 31 },
    "equipment.serva.eq_31_back": { label: "equipment.serva.eq_31_back", width: 451.02, height: 1381.60, size: 31 },
    "equipment.serva.eq_32": { label: "equipment.serva.eq_32", width: 500.02, height: 1424.08, size: 32 },
    "equipment.serva.eq_32_back": { label: "equipment.serva.eq_32_back", width: 451.02, height: 1425.10, size: 32 },
    "equipment.serva.eq_33": { label: "equipment.serva.eq_33", width: 500.02, height: 1468.60, size: 33 },
    "equipment.serva.eq_33_back": { label: "equipment.serva.eq_33_back", width: 451.02, height: 1470.33, size: 33 },
    "equipment.serva.eq_34": { label: "equipment.serva.eq_34", width: 500.02, height: 1513.09, size: 34 },
    "equipment.serva.eq_34_back": { label: "equipment.serva.eq_34_back", width: 451.02, height: 1514.11, size: 34 },
    "equipment.serva.eq_35": { label: "equipment.serva.eq_35", width: 500.02, height: 1557.61, size: 35 },
    "equipment.serva.eq_3": { label: "equipment.serva.eq_3", width: 500.02, height: 133.50, size: 3 },
    "equipment.serva.eq_36": { label: "equipment.serva.eq_36", width: 500.02, height: 1602.10, size: 36 },
    "equipment.serva.eq_35_back": { label: "equipment.serva.eq_35_back", width: 450.88, height: 1557.08, size: 35 },
    "equipment.serva.eq_37": { label: "equipment.serva.eq_37", width: 500.02, height: 1646.59, size: 37 },
    "equipment.serva.eq_36_back": { label: "equipment.serva.eq_36_back", width: 451.02, height: 1603.09, size: 36 },
    "equipment.serva.eq_38": { label: "equipment.serva.eq_38", width: 500.02, height: 1691.11, size: 38 },
    "equipment.serva.eq_37_back": { label: "equipment.serva.eq_37_back", width: 450.88, height: 1647.61, size: 37 },
    "equipment.serva.eq_38_back": { label: "equipment.serva.eq_38_back", width: 451.02, height: 1692.10, size: 38 },
    "equipment.serva.eq_39": { label: "equipment.serva.eq_39", width: 500.02, height: 1735.60, size: 39 },
    "equipment.serva.eq_39_back": { label: "equipment.serva.eq_39_back", width: 451.05, height: 1736.62, size: 39 },
    "equipment.serva.eq_4": { label: "equipment.serva.eq_4", width: 500.02, height: 178.02, size: 4 },
    "equipment.serva.eq_40": { label: "equipment.serva.eq_40", width: 500.02, height: 1780.12, size: 40 },
    "equipment.serva.eq_40_back": { label: "equipment.serva.eq_40_back", width: 451.05, height: 1781.11, size: 40 },
    "equipment.serva.eq_41": { label: "equipment.serva.eq_41", width: 500.02, height: 1824.61, size: 41 },
    "equipment.serva.eq_41_back": { label: "equipment.serva.eq_41_back", width: 451.05, height: 1825.60, size: 41 },
    "equipment.serva.eq_42": { label: "equipment.serva.eq_42", width: 500.02, height: 1869.13, size: 42 },
    "equipment.serva.eq_42_back": { label: "equipment.serva.eq_42_back", width: 451.05, height: 1870.12, size: 42 },
    "equipment.serva.eq_43": { label: "equipment.serva.eq_43", width: 500.02, height: 1913.62, size: 43 },
    "equipment.serva.eq_2_back": { label: "equipment.serva.eq_2_back", width: 451.02, height: 90.00, size: 2 },
    "equipment.serva.eq_43_back": { label: "equipment.serva.eq_43_back", width: 451.02, height: 1914.61, size: 43 },
    "equipment.serva.eq_44": { label: "equipment.serva.eq_44", width: 500.02, height: 1958.11, size: 44 },
    "equipment.serva.eq_45": { label: "equipment.serva.eq_45", width: 500.02, height: 2002.63, size: 45 },
    "equipment.serva.eq_44_back": { label: "equipment.serva.eq_44_back", width: 451.02, height: 1959.13, size: 44 },
    "equipment.serva.eq_45_back": { label: "equipment.serva.eq_45_back", width: 451.02, height: 2003.62, size: 45 },
    "equipment.serva.eq_46": { label: "equipment.serva.eq_46", width: 500.02, height: 2047.12, size: 46 },
    "equipment.serva.eq_47": { label: "equipment.serva.eq_47", width: 500.02, height: 2091.65, size: 47 },
    "equipment.serva.eq_46_back": { label: "equipment.serva.eq_46_back", width: 451.02, height: 2048.15, size: 46 },
    "equipment.serva.eq_4_back": { label: "equipment.serva.eq_4_back", width: 451.02, height: 179.01, size: 4 },
    "equipment.serva.eq_47_back": { label: "equipment.serva.eq_47_back", width: 451.02, height: 2092.63, size: 47 },
    "equipment.serva.eq_5": { label: "equipment.serva.eq_5", width: 500.02, height: 222.51, size: 5 },
    "equipment.serva.eq_5_back": { label: "equipment.serva.eq_5_back", width: 451.02, height: 222.48, size: 5 },
    "equipment.serva.eq_6_back": { label: "equipment.serva.eq_6_back", width: 451.02, height: 268.02, size: 6 },
    "equipment.serva.eq_6": { label: "equipment.serva.eq_6", width: 500.02, height: 267.03, size: 6 },
    "equipment.serva.eq_7_back": { label: "equipment.serva.eq_7_back", width: 451.02, height: 312.51, size: 7 },
    "equipment.serva.eq_7": { label: "equipment.serva.eq_7", width: 500.02, height: 311.52, size: 7 },
    "equipment.serva.eq_8_back": { label: "equipment.serva.eq_8_back", width: 451.02, height: 357.03, size: 8 },
    "equipment.serva.eq_8": { label: "equipment.serva.eq_8", width: 500.02, height: 356.01, size: 8 },
    "equipment.serva.eq_1_back": { label: "equipment.serva.eq_1_back", width: 451.02, height: 45.51, size: 1 },
    "equipment.serva.eq_3_back": { label: "equipment.serva.eq_3_back", width: 450.63, height: 134.10, size: 3 },
    "equipment.venus.2453": { label: "equipment.venus.2453", width: 500.27, height: 89.01, size: 2 },
    "equipment.venus.2453_back": { label: "equipment.venus.2453_back", width: 450.03, height: 89.33, size: 2 },
    "equipment.venus.adm_guard_6800n_m_back": { label: "equipment.venus.adm_guard_6800n_m_back", width: 450.03, height: 89.01, size: 2 },
    "equipment.venus.2454_back": { label: "equipment.venus.2454_back", width: 450.03, height: 89.01, size: 2 },
    "equipment.venus.adm_guard_6800n_m": { label: "equipment.venus.adm_guard_6800n_m", width: 500.27, height: 89.01, size: 2 },
    "equipment.huawei.rzl_hw_back": { label: "equipment.huawei.rzl_hw_back", width: 450.14, height: 134.88, size: 3 },
    "equipment.venus.cjz-cbs_a2167n_back": { label: "equipment.venus.cjz-cbs_a2167n_back", width: 450.03, height: 89.01, size: 2 },
    "equipment.venus.cj_m9500_back": { label: "equipment.venus.cj_m9500_back", width: 450.03, height: 89.01, size: 2 },
    "equipment.venus.cjz-cbs_a2167n": { label: "equipment.venus.cjz-cbs_a2167n", width: 500.02, height: 89.01, size: 2 },
    "equipment.venus.cj_m9500": { label: "equipment.venus.cj_m9500", width: 500.02, height: 89.01, size: 2 },
    "equipment.venus.lbg_6006": { label: "equipment.venus.lbg_6006", width: 500.27, height: 89.01, size: 2 },
    "equipment.venus.lbg_6006_back": { label: "equipment.venus.lbg_6006_back", width: 450.77, height: 89.12, size: 2 },
    "equipment.venus.2454": { label: "equipment.venus.2454", width: 500.27, height: 89.01, size: 2 },
    "equipment.huawei.rzl_hw": { label: "equipment.huawei.rzl_hw", width: 500.24, height: 133.50, size: 3 },
    "equipment.aether.afw2000": { label: "equipment.aether.afw2000", width: 500.02, height: 44.49, size: 1 },
    "equipment.aether.asg1000_back": { label: "equipment.aether.asg1000_back", width: 450.03, height: 89.01, size: 2 },
    "equipment.aether.asg1000": { label: "equipment.aether.asg1000", width: 450.03, height: 89.01, size: 2 },
    "equipment.aether.afw2000_back": { label: "equipment.aether.afw2000_back", width: 450.03, height: 44.49, size: 1 },
    "equipment.aether.eme4280": { label: "equipment.aether.eme4280", width: 450.03, height: 178.02, size: 4 },
    "equipment.aether.net2000b": { label: "equipment.aether.net2000b", width: 500.02, height: 44.49, size: 1 },
    "equipment.aether.scp2000e": { label: "equipment.aether.scp2000e", width: 500.02, height: 89.01, size: 2 },
    "equipment.aether.net2000b_back": { label: "equipment.aether.net2000b_back", width: 450.03, height: 44.49, size: 1 },
    "equipment.aether.eme4280_back": { label: "equipment.aether.eme4280_back", width: 450.03, height: 178.02, size: 4 },
    "equipment.array.apv_3520": { label: "equipment.array.apv_3520", width: 500.02, height: 89.01, size: 2 },
    "equipment.aether.scp2000e_back": { label: "equipment.aether.scp2000e_back", width: 450.03, height: 89.01, size: 2 },
    "equipment.array.apv_3520_back": { label: "equipment.array.apv_3520_back", width: 450.03, height: 89.01, size: 2 },
    "equipment.cisco.2769": { label: "equipment.cisco.2769", width: 500.02, height: 44.49, size: 1 },
    "equipment.cisco.2960g": { label: "equipment.cisco.2960g", width: 500.02, height: 44.49, size: 1 },
    "equipment.cisco.2769_back": { label: "equipment.cisco.2769_back", width: 450.03, height: 44.49, size: 1 },
    "equipment.cisco.2960g_back": { label: "equipment.cisco.2960g_back", width: 450.03, height: 44.49, size: 1 },
    "equipment.cisco.2960_s": { label: "equipment.cisco.2960_s", width: 500.02, height: 44.49, size: 1 },
    "equipment.cisco.7000_2": { label: "equipment.cisco.7000_2", width: 450.03, height: 934.57, size: 21 },
    "equipment.cisco.7606_s": { label: "equipment.cisco.7606_s", width: 450.03, height: 311.52, size: 7 },
    "equipment.cisco.7606_s_back": { label: "equipment.cisco.7606_s_back", width: 450.03, height: 311.52, size: 7 },
    "equipment.citrix.2764_back": { label: "equipment.citrix.2764_back", width: 450.03, height: 89.01, size: 2 },
    "equipment.citrix.2764": { label: "equipment.citrix.2764", width: 500.02, height: 89.01, size: 2 },
    "equipment.cisco.cisco_3900_back": { label: "equipment.cisco.cisco_3900_back", width: 450.03, height: 133.50, size: 3 },
    "equipment.citrix.2781": { label: "equipment.citrix.2781", width: 500.02, height: 44.49, size: 1 },
    "equipment.citrix.2781_back": { label: "equipment.citrix.2781_back", width: 450.03, height: 44.49, size: 1 },
    "equipment.digitalchina.dcs_4500": { label: "equipment.digitalchina.dcs_4500", width: 500.02, height: 44.49, size: 1 },
    "equipment.digitalchina.dcs_4500_back": { label: "equipment.digitalchina.dcs_4500_back", width: 450.03, height: 44.49, size: 1 },
    "equipment.dell.2800": { label: "equipment.dell.2800", width: 225.02, height: 489.55, size: 11 },
    "equipment.cisco.7000_2_back": { label: "equipment.cisco.7000_2_back", width: 450.03, height: 934.57, size: 21 },
    "equipment.h3c.a2020": { label: "equipment.h3c.a2020", width: 500.02, height: 44.49, size: 1 },
    "equipment.h3c.f1000f_s_ai": { label: "equipment.h3c.f1000f_s_ai", width: 500.02, height: 44.49, size: 1 },
    "equipment.h3c.2682": { label: "equipment.h3c.2682", width: 500.02, height: 89.01, size: 2 },
    "equipment.h3c.f1000f_s_ai_back": { label: "equipment.h3c.f1000f_s_ai_back", width: 450.03, height: 44.49, size: 1 },
    "equipment.h3c.s3110": { label: "equipment.h3c.s3110", width: 500.02, height: 44.49, size: 1 },
    "equipment.dell.2800_back": { label: "equipment.dell.2800_back", width: 225.02, height: 489.55, size: 11 },
    "equipment.h3c.s5560_1": { label: "equipment.h3c.s5560_1", width: 450.03, height: 44.49, size: 1 },
    "equipment.h3c.s5560_1_back": { label: "equipment.h3c.s5560_1_back", width: 500.02, height: 44.49, size: 1 },
    "equipment.h3c.s5800_back": { label: "equipment.h3c.s5800_back", width: 450.03, height: 44.49, size: 1 },
    "equipment.h3c.S12508": { label: "equipment.h3c.S12508", width: 450.03, height: 979.06, size: 22 },
    "equipment.h3c.s5800": { label: "equipment.h3c.s5800", width: 500.02, height: 44.49, size: 1 },
    "equipment.h3c.s7508e_x": { label: "equipment.h3c.s7508e_x", width: 500.02, height: 623.04, size: 14 },
    "equipment.h3c.s7508e_x_back": { label: "equipment.h3c.s7508e_x_back", width: 500.02, height: 621.99, size: 14 },
    "equipment.h3c.S12508_back": { label: "equipment.h3c.S12508_back", width: 407.20, height: 979.06, size: 22 },
    "equipment.h3c.t1000_s": { label: "equipment.h3c.t1000_s", width: 500.02, height: 89.01, size: 2 },
    "equipment.h3c.socpath_m300": { label: "equipment.h3c.socpath_m300", width: 450.03, height: 890.04, size: 20 },
    "equipment.h3c.socpath_m300_back": { label: "equipment.h3c.socpath_m300_back", width: 450.03, height: 890.04, size: 20 },
    "equipment.ibm.2724": { label: "equipment.ibm.2724", width: 450.03, height: 178.02, size: 4 },
    "equipment.h3c.t1000_s_back": { label: "equipment.h3c.t1000_s_back", width: 500.02, height: 89.01, size: 2 },
    "equipment.ibm.2724_back": { label: "equipment.ibm.2724_back", width: 450.03, height: 178.02, size: 4 },
    "equipment.ibm.550": { label: "equipment.ibm.550", width: 450.03, height: 178.02, size: 4 },
    "equipment.ibm.ds5300": { label: "equipment.ibm.ds5300", width: 450.03, height: 311.52, size: 7 },
    "equipment.ibm.550_back": { label: "equipment.ibm.550_back", width: 450.03, height: 178.02, size: 4 },
    "equipment.westone.sjy15_c_1_back": { label: "equipment.westone.sjy15_c_1_back", width: 500.02, height: 178.02, size: 4 },
    "equipment.westone.sjy15_c_1": { label: "equipment.westone.sjy15_c_1", width: 500.02, height: 178.02, size: 4 },
    "equipment.ibm.p750_back": { label: "equipment.ibm.p750_back", width: 450.14, height: 222.69, size: 5 },
    "equipment.cisco.c3kx_nm_1g-2_back": { label: "equipment.cisco.c3kx_nm_1g_back", width: 450.03, height: 44.77, size: 1 },
    "equipment.cisco.c3kx_nm_1g-2": { label: "equipment.cisco.c3kx_nm_1g", width: 450.03, height: 44.49, size: 1 },
    "equipment.cisco.c3kx_nm_1g_back": { label: "equipment.cisco.c3kx_nm_1g_back", width: 450.03, height: 44.77, size: 1 },
    "equipment.cisco.c3kx_nm_1g": { label: "equipment.cisco.c3kx_nm_1g", width: 450.03, height: 44.49, size: 1 },
    "equipment.cisco.ucs_c220_m3": { label: "equipment.cisco.ucs_c220_m3", width: 450.03, height: 44.49, size: 1 },
    "equipment.cisco.ucs_c220_m3_back": { label: "equipment.cisco.ucs_c220_m3_back", width: 450.03, height: 44.49, size: 1 },
    "equipment.cisco.2763_back": { label: "equipment.cisco.2763_back", width: 450.10, height: 222.51, size: 5 },
    "equipment.hitachi.g200_back": { label: "equipment.hitachi.g200_back", width: 450.03, height: 89.01, size: 2 },
    "equipment.hp.2744": { label: "equipment.hp.2744", width: 450.03, height: 400.53, size: 9 },
    "equipment.hitachi.g200": { label: "equipment.hitachi.g200", width: 450.03, height: 89.61, size: 2 },
    "equipment.cisco.2763": { label: "equipment.cisco.2763", width: 500.02, height: 222.51, size: 5 },
    "equipment.hp.2744_back": { label: "equipment.hp.2744_back", width: 450.03, height: 400.75, size: 9 },
    "equipment.hp.2745_back": { label: "equipment.hp.2745_back", width: 450.03, height: 934.57, size: 21 },
    "equipment.hp.2745": { label: "equipment.hp.2745", width: 450.03, height: 934.57, size: 21 },
    "equipment.hp.StoageWorks_back": { label: "equipment.hp.StoageWorks_back", width: 450.03, height: 133.50, size: 3 },
    "equipment.hp.5300": { label: "equipment.hp.5300", width: 450.03, height: 133.53, size: 3 },
    "equipment.hp.5300_back": { label: "equipment.hp.5300_back", width: 450.03, height: 133.50, size: 3 },
    "equipment.huawei.rh2288V3_back": { label: "equipment.huawei.rh2288V3_back", width: 450.03, height: 89.01, size: 2 },
    "equipment.ibm.2743": { label: "equipment.ibm.2743", width: 450.03, height: 178.02, size: 4 },
    "equipment.huawei.sns2248": { label: "equipment.huawei.sns2248", width: 500.02, height: 44.49, size: 1 },
    "equipment.huawei.sns2248_back": { label: "equipment.huawei.sns2248_back", width: 450.03, height: 44.49, size: 1 },
    "equipment.ibm.2776": { label: "equipment.ibm.2776", width: 499.32, height: 44.49, size: 1 },
    "equipment.ibm.2776_back": { label: "equipment.ibm.2776_back", width: 500.02, height: 44.49, size: 1 },
    "equipment.ibm.2783_back": { label: "equipment.ibm.2783_back", width: 450.03, height: 44.49, size: 1 },
    "equipment.ibm.2783": { label: "equipment.ibm.2783", width: 500.02, height: 44.49, size: 1 },
    "equipment.ibm.2784": { label: "equipment.ibm.2784", width: 450.03, height: 44.49, size: 1 },
    "equipment.ibm.ds3512": { label: "equipment.ibm.ds3512", width: 500.02, height: 87.85, size: 2 },
    "equipment.ibm.2784_back": { label: "equipment.ibm.2784_back", width: 450.03, height: 44.49, size: 1 },
    "equipment.ibm.3650-M2_back": { label: "equipment.ibm.3650-M2_back", width: 450.03, height: 89.01, size: 2 },
    "equipment.ibm.DS5020": { label: "equipment.ibm.DS5020", width: 500.02, height: 133.50, size: 3 },
    "equipment.ibm.ds3512_back": { label: "equipment.ibm.ds3512_back", width: 450.03, height: 89.01, size: 2 },
    "equipment.ibm.ibm_2498_b24_back": { label: "equipment.ibm.ibm_2498_b24_back", width: 450.03, height: 44.49, size: 1 },
    "equipment.ibm.DS5020_back": { label: "equipment.ibm.DS5020_back", width: 450.03, height: 133.50, size: 3 },
    "equipment.ibm.ibm_2498_b24": { label: "equipment.ibm.ibm_2498_b24", width: 500.02, height: 44.49, size: 1 },
    "equipment.ibm.ibm_v5000_back": { label: "equipment.ibm.ibm_v5000_back", width: 450.03, height: 133.50, size: 3 },
    "equipment.ibm.p5": { label: "equipment.ibm.p5", width: 450.03, height: 178.02, size: 4 },
    "equipment.ibm.ibm_x3750_m4_back": { label: "equipment.ibm.ibm_x3750_m4_back", width: 450.03, height: 89.01, size: 2 },
    "equipment.ibm.system_3650_back": { label: "equipment.ibm.system_3650_back", width: 450.03, height: 89.01, size: 2 },
    "equipment.ibm.rs6000": { label: "equipment.ibm.rs6000", width: 500.02, height: 400.89, size: 9 },
    "equipment.ibm.p5_back": { label: "equipment.ibm.p5_back", width: 450.03, height: 177.81, size: 4 },
    "equipment.ibm.rzl_ibm_back": { label: "equipment.ibm.rzl_ibm_back", width: 450.03, height: 445.02, size: 10 },
    "equipment.ibm.v3500": { label: "equipment.ibm.v3500", width: 450.03, height: 89.01, size: 2 },
    "equipment.ibm.v3500_back": { label: "equipment.ibm.v3500_back", width: 500.02, height: 89.01, size: 2 },
    "equipment.ibm.v7000": { label: "equipment.ibm.v7000", width: 500.02, height: 89.01, size: 2 },
    "equipment.inspur.nf560d2_back": { label: "equipment.inspur.nf560d2_back", width: 450.03, height: 178.16, size: 4 },
    "equipment.lenovo.2758": { label: "equipment.lenovo.2758", width: 500.02, height: 44.49, size: 1 },
    "equipment.lenovo.ls_ip_1100": { label: "equipment.lenovo.ls_ip_1100", width: 500.02, height: 44.49, size: 1 },
    "equipment.pbdata.2756": { label: "equipment.pbdata.2756", width: 500.02, height: 89.01, size: 2 },
    "equipment.lenovo.sr860_back": { label: "equipment.lenovo.sr860_back", width: 450.03, height: 89.01, size: 2 },
    "equipment.lenovo.sr860": { label: "equipment.lenovo.sr860", width: 500.02, height: 89.01, size: 2 },
    "equipment.netapp.2674": { label: "equipment.netapp.2674", width: 500.02, height: 89.01, size: 2 },
    "equipment.netapp.2674_back": { label: "equipment.netapp.2674_back", width: 450.03, height: 89.01, size: 2 },
    "equipment.qlogic.2732": { label: "equipment.qlogic.2732", width: 450.03, height: 44.49, size: 1 },
    "equipment.qlogic.2732_back": { label: "equipment.qlogic.2732_back", width: 450.03, height: 44.49, size: 1 },
    "equipment.pbdata.2756_back": { label: "equipment.pbdata.2756_back", width: 450.03, height: 89.01, size: 2 },
    "equipment.radware.1016_back": { label: "equipment.radware.1016_back", width: 450.03, height: 44.49, size: 1 },
    "equipment.radware.1016": { label: "equipment.radware.1016", width: 500.02, height: 44.49, size: 1 },
    "equipment.radware.2668": { label: "equipment.radware.2668", width: 500.02, height: 89.01, size: 2 },
    "equipment.radware.2668_back": { label: "equipment.radware.2668_back", width: 450.03, height: 89.01, size: 2 },
    "equipment.sangfor.ad_2000_xt": { label: "equipment.sangfor.ad_2000_xt", width: 500.02, height: 89.01, size: 2 },
    "equipment.sangfor.ad_2000_xt_back": { label: "equipment.sangfor.ad_2000_xt_back", width: 450.03, height: 89.01, size: 2 },
    "equipment.serva.2766": { label: "equipment.serva.2766", width: 500.02, height: 178.02, size: 4 },
    "equipment.serva.2766_back": { label: "equipment.serva.2766_back", width: 450.03, height: 178.02, size: 4 },
    "equipment.ibm.ibm_x3650_m3_back": { label: "equipment.ibm.ibm_x3650_m3_back", width: 450.14, height: 88.94, size: 2 },
    "equipment.sangfor.ad_2000_xt_2": { label: "equipment.sangfor.ad_2000_xt_2", width: 500.02, height: 89.01, size: 2 },
    "equipment.sangfor.ad_3000": { label: "equipment.sangfor.ad_3000", width: 500.02, height: 89.01, size: 2 },
    "equipment.sangfor.af_1520_tb": { label: "equipment.sangfor.af_1520_tb", width: 500.02, height: 44.49, size: 1 },
    "equipment.sangfor.ad_2000_xt_2_back": { label: "equipment.sangfor.ad_2000_xt_2_back", width: 450.03, height: 89.01, size: 2 },
    "equipment.sangfor.ad_3000_back": { label: "equipment.sangfor.ad_3000_back", width: 450.03, height: 89.01, size: 2 },
    "equipment.sangfor.vds_5050": { label: "equipment.sangfor.vds_5050", width: 500.02, height: 89.01, size: 2 },
    "equipment.sangfor.af_1520_tb_back": { label: "equipment.sangfor.af_1520_tb_back", width: 450.03, height: 44.49, size: 1 },
    "equipment.serva.2667": { label: "equipment.serva.2667", width: 450.03, height: 44.49, size: 1 },
    "equipment.sangfor.vds_5050_back": { label: "equipment.sangfor.vds_5050_back", width: 458.89, height: 89.01, size: 2 },
    "equipment.serva.2667_back": { label: "equipment.serva.2667_back", width: 450.03, height: 44.49, size: 1 },
    "equipment.serva.2772_back": { label: "equipment.serva.2772_back", width: 500.02, height: 44.49, size: 1 },
    "equipment.serva.2772": { label: "equipment.serva.2772", width: 450.03, height: 44.49, size: 1 },
    "equipment.serva.2780": { label: "equipment.serva.2780", width: 450.03, height: 222.51, size: 5 },
    "equipment.serva.2780_back": { label: "equipment.serva.2780_back", width: 450.03, height: 222.51, size: 5 },
    "equipment.serva.2779": { label: "equipment.serva.2779", width: 450.03, height: 534.03, size: 12 },
    "equipment.serva.2779_back": { label: "equipment.serva.2779_back", width: 450.10, height: 534.03, size: 12 },
    "equipment.serva.kvm_back": { label: "equipment.serva.kvm_back", width: 450.03, height: 44.49, size: 1 },
    "equipment.serva.kvm": { label: "equipment.serva.kvm", width: 500.02, height: 44.49, size: 1 },
    "equipment.srie.2673": { label: "equipment.srie.2673", width: 500.02, height: 89.01, size: 2 },
    "equipment.srie.2673_back": { label: "equipment.srie.2673_back", width: 458.89, height: 89.01, size: 2 },
    "equipment.srie.2750": { label: "equipment.srie.2750", width: 500.02, height: 89.01, size: 2 },
    "equipment.srie.2750_back": { label: "equipment.srie.2750_back", width: 450.03, height: 89.01, size: 2 },
    "equipment.srie.2755": { label: "equipment.srie.2755", width: 450.03, height: 89.01, size: 2 },
    "equipment.srie.2754": { label: "equipment.srie.2754", width: 500.02, height: 89.01, size: 2 },
    "equipment.srie.2755_back": { label: "equipment.srie.2755_back", width: 500.02, height: 89.01, size: 2 },
    "equipment.srie.ndp1000": { label: "equipment.srie.ndp1000", width: 500.02, height: 44.49, size: 1 },
    "equipment.srie.ndp1000_2": { label: "equipment.srie.ndp1000_2", width: 500.02, height: 44.49, size: 1 },
    "equipment.srie.smartsign": { label: "equipment.srie.smartsign", width: 499.67, height: 44.45, size: 1 },
    "equipment.srie.smartsign_back": { label: "equipment.srie.smartsign_back", width: 450.03, height: 44.49, size: 1 },
    "equipment.topsec.ngfw4000_uf": { label: "equipment.topsec.ngfw4000_uf", width: 450.03, height: 44.49, size: 1 },
    "equipment.topsec.ngfw4000_uf_back": { label: "equipment.topsec.ngfw4000_uf_back", width: 500.02, height: 44.49, size: 1 },
    "equipment.wangshen.2775": { label: "equipment.wangshen.2775", width: 500.02, height: 44.49, size: 1 },
    "equipment.wangshen.2775_back": { label: "equipment.wangshen.2775_back", width: 450.03, height: 44.49, size: 1 },
    "equipment.westone.sjy15_c": { label: "equipment.westone.sjy15_c", width: 500.02, height: 89.01, size: 2 },
    "equipment.westone.sjy15_c_back": { label: "equipment.westone.sjy15_c_back", width: 450.03, height: 89.01, size: 2 },
    "equipment.ibm.v7000_back": { label: "equipment.ibm.v7000_back", width: 449.82, height: 88.94, size: 2 },
    "quipment.dell.2650_back": { label: "quipment.dell.2650_back", width: 450.03, height: 89.01, size: 2 },
    "quipment.dell.2650": { label: "quipment.dell.2650", width: 450.03, height: 89.01, size: 2 },
    "equipment.serva.2786": { label: "equipment.serva.2786", width: 450.03, height: 890.04, size: 20 },
    "equipment.serva.2786_back": { label: "equipment.serva.2786_back", width: 450.03, height: 890.04, size: 20 },
    "equipment.aether.scp2000s": { label: "equipment.aether.scp2000s", width: 500.02, height: 44.49, size: 1 },
    "equipment.aether.yd_node2000": { label: "equipment.aether.yd_node2000", width: 500.02, height: 44.49, size: 1 },
    "equipment.aether.scp2000s_back": { label: "equipment.aether.scp2000s_back", width: 450.03, height: 44.49, size: 1 },
    "equipment.aether.yd_node2000_back": { label: "equipment.aether.yd_node2000_back", width: 450.03, height: 44.49, size: 1 },
    "equipment.airfact.node2000e_back": { label: "equipment.airfact.node2000e_back", width: 500.02, height: 89.01, size: 2 },
    "equipment.airfact.node2000e": { label: "equipment.airfact.node2000e", width: 450.03, height: 89.01, size: 2 },
    "equipment.cisco.2960g_1": { label: "equipment.cisco.2960g_1", width: 500.02, height: 44.49, size: 1 },
    "equipment.cisco.7606-s": { label: "equipment.cisco.7606-s", width: 450.46, height: 311.52, size: 8 },
    "equipment.cisco.2960g_1_back": { label: "equipment.cisco.2960g_1_back", width: 450.03, height: 44.49, size: 1 },
    "equipment.cisco.7606-s_back": { label: "equipment.cisco.7606-s_back", width: 450.03, height: 311.52, size: 8 },
    "equipment.h3c.sr6604_x": { label: "equipment.h3c.sr6604_x", width: 450.03, height: 222.51, size: 5 },
    "equipment.h3c.sr6604_x_2": { label: "equipment.h3c.sr6604_x_2", width: 450.03, height: 222.51, size: 5 },
    "equipment.h3c.sr6604_x_back": { label: "equipment.h3c.sr6604_x_back", width: 450.03, height: 222.51, size: 5 },
    "equipment.h3c.sr6604_x_2_back": { label: "equipment.h3c.sr6604_x_2_back", width: 450.03, height: 222.51, size: 5 },
    "equipment.h3c.sr8804_x": { label: "equipment.h3c.sr8804_x", width: 449.08, height: 356.01, size: 8 },
    "equipment.h3c.sr8804_x_back": { label: "equipment.h3c.sr8804_x_back", width: 450.03, height: 356.01, size: 8 },
    "equipment.hp.2687_back": { label: "equipment.hp.2687_back", width: 451.02, height: 445.02, size: 10 },
    "equipment.hp.2687": { label: "equipment.hp.2687", width: 450.03, height: 445.02, size: 10 },
    "equipment.huawei.2654": { label: "equipment.huawei.2654", width: 450.03, height: 178.02, size: 4 },
    "equipment.huawei.uap3000": { label: "equipment.huawei.uap3000", width: 450.03, height: 267.03, size: 6 },
    "equipment.huawei.2654_back": { label: "equipment.huawei.2654_back", width: 450.03, height: 178.02, size: 4 },
    "equipment.huawei.uap3000_back": { label: "equipment.huawei.uap3000_back", width: 450.74, height: 267.74, size: 6 },
    "equipment.hp.963_back": { label: "equipment.hp.963_back", width: 450.03, height: 177.49, size: 4 },
    "equipment.cisco.7609_back": { label: "equipment.cisco.7609_back", width: 450.03, height: 934.57, size: 21 },
    "equipment.cisco.7609": { label: "equipment.cisco.7609", width: 450.17, height: 934.57, size: 21 },
    "equipment.cisco.7609_s_back": { label: "equipment.cisco.7609_s_back", width: 450.03, height: 756.58, size: 17 },
    "equipment.cisco.7609_s": { label: "equipment.cisco.7609_s", width: 449.68, height: 756.54, size: 17 },
    "equipment.huawei.s6720_54c_ei_48s_ac": { label: "equipment.huawei.s6720_54c_ei_48s_ac", width: 482.28, height: 44.45, size: 1 },
    "equipment.huawei.s6720_54c_ei_48s_ac-2": { label: "equipment.huawei.s6720_54c_ei_48s_ac-2", width: 482.28, height: 44.45, size: 1 },
    "equipment.brocade.300": { label: "equipment.brocade.300", width: 500.02, height: 44.49, size: 1 },
    "equipment.brocade.300_back": { label: "equipment.brocade.300_back", width: 500.02, height: 44.49, size: 1 },
    "equipment.cisco.dtst_cisco5520": { label: "equipment.cisco.dtst_cisco5520", width: 500.02, height: 44.49, size: 1 },
    "equipment.h3c.h3c_s6800": { label: "equipment.h3c.h3c_s6800", width: 500.02, height: 89.01, size: 2 },
    "equipment.h3c.ns_secpath_t5000_s": { label: "equipment.h3c.ns_secpath_t5000_s", width: 500.02, height: 89.01, size: 2 },
    "equipment.h3c.h3c_s6800_back": { label: "equipment.h3c.h3c_s6800_back", width: 500.02, height: 89.01, size: 2 },
    "equipment.h3c.ns_secpath_t5000_s_back": { label: "equipment.h3c.ns_secpath_t5000_s_back", width: 500.02, height: 89.01, size: 2 },
    "equipment.hp.hp_380_g5": { label: "equipment.hp.hp_380_g5", width: 500.34, height: 89.01, size: 2 },
    "equipment.hp.hp_kvm": { label: "equipment.hp.hp_kvm", width: 500.02, height: 44.49, size: 1 },
    "equipment.hp.hp_380_g5_back": { label: "equipment.hp.hp_380_g5_back", width: 500.34, height: 89.01, size: 2 },
    "equipment.hp.hp_rx3600": { label: "equipment.hp.hp_rx3600", width: 500.02, height: 89.01, size: 2 },
    "equipment.huawei.rh2288h_v23_3": { label: "equipment.huawei.rh2288h_v23_3", width: 500.02, height: 89.01, size: 2 },
    "equipment.huawei.rh2288h_v23_3_back": { label: "equipment.huawei.rh2288h_v23_3_back", width: 450.17, height: 89.01, size: 2 },
    "equipment.ibm.2498": { label: "equipment.ibm.2498", width: 500.02, height: 44.49, size: 1 },
    "equipment.ibm.1168": { label: "equipment.ibm.1168", width: 220.01, height: 445.02, size: 10 },
    "equipment.ibm.2498_back": { label: "equipment.ibm.2498_back", width: 500.02, height: 44.49, size: 1 },
    "equipment.ibm.707_back": { label: "equipment.ibm.707_back", width: 500.02, height: 133.50, size: 3 },
    "equipment.ibm.707": { label: "equipment.ibm.707", width: 500.02, height: 133.50, size: 3 },
    "equipment.ibm.dtst_ibmv7000_back": { label: "equipment.ibm.dtst_ibmv7000_back", width: 450.03, height: 89.01, size: 2 },
    "equipment.ibm.dtst_ibmv7000": { label: "equipment.ibm.dtst_ibmv7000", width: 500.02, height: 89.01, size: 2 },
    "equipment.ibm.ibm-3650M3": { label: "equipment.ibm.ibm-3650M3", width: 500.02, height: 89.01, size: 2 },
    "equipment.ibm.ds5300_back": { label: "equipment.ibm.ds5300_back", width: 450.17, height: 311.52, size: 7 },
    "equipment.ibm.ibm-3650M3_back": { label: "equipment.ibm.ibm-3650M3_back", width: 450.03, height: 89.01, size: 2 },
    "equipment.ibm.ibm_7214_1u2": { label: "equipment.ibm.ibm_7214_1u2", width: 500.02, height: 44.49, size: 1 },
    "equipment.ibm.ibm_7214_1u2_back": { label: "equipment.ibm.ibm_7214_1u2_back", width: 500.02, height: 44.49, size: 1 },
    "equipment.ibm.ibm_7316_tf3": { label: "equipment.ibm.ibm_7316_tf3", width: 500.02, height: 44.49, size: 1 },
    "equipment.ibm.ibm88524yc_back": { label: "equipment.ibm.ibm88524yc_back", width: 450.03, height: 445.02, size: 10 },
    "equipment.ibm.ibm_ps814": { label: "equipment.ibm.ibm_ps814", width: 500.06, height: 178.02, size: 4 },
    "equipment.ibm.ibm_x3550": { label: "equipment.ibm.ibm_x3550", width: 450.17, height: 44.49, size: 1 },
    "equipment.ibm.ibm_x3250_m5": { label: "equipment.ibm.ibm_x3250_m5", width: 500.02, height: 44.49, size: 1 },
    "equipment.ibm.ibm_x3550_back": { label: "equipment.ibm.ibm_x3550_back", width: 450.17, height: 44.49, size: 1 },
    "equipment.ibm.ibm_x3650_m4_back": { label: "equipment.ibm.ibm_x3650_m4_back", width: 450.17, height: 89.01, size: 2 },
    "equipment.ibm.ibm88524yc": { label: "equipment.ibm.ibm88524yc", width: 500.02, height: 445.02, size: 10 },
    "equipment.ibm.ibm_x3650_m5_back": { label: "equipment.ibm.ibm_x3650_m5_back", width: 450.17, height: 89.01, size: 2 },
    "equipment.ibm.ibm_x3550_m4_back": { label: "equipment.ibm.ibm_x3550_m4_back", width: 450.03, height: 44.49, size: 1 },
    "equipment.ibm.ibm_x3850_x5": { label: "equipment.ibm.ibm_x3850_x5", width: 499.92, height: 178.02, size: 4 },
    "equipment.ibm.ibm_x3850_x5_back": { label: "equipment.ibm.ibm_x3850_x5_back", width: 450.17, height: 178.02, size: 4 },
    "equipment.ibm.ibm_x3850_x6_back": { label: "equipment.ibm.ibm_x3850_x6_back", width: 450.17, height: 178.02, size: 4 },
    "equipment.ibm.p750": { label: "equipment.ibm.p750", width: 499.92, height: 178.02, size: 4 },
    "equipment.ibm.SYSTEMP5": { label: "equipment.ibm.SYSTEMP5", width: 500.02, height: 178.02, size: 4 },
    "equipment.ibm.system_3650": { label: "equipment.ibm.system_3650", width: 500.06, height: 89.01, size: 2 },
    "equipment.ibm.rzl_ibm": { label: "equipment.ibm.rzl_ibm", width: 500.02, height: 445.02, size: 10 },
    "equipment.lenovo.rq940": { label: "equipment.lenovo.rq940", width: 500.02, height: 178.02, size: 4 },
    "equipment.lenovo.rq940_back": { label: "equipment.lenovo.rq940_back", width: 450.03, height: 178.02, size: 4 },
    "equipment.ibm.SYSTEMP5_back": { label: "equipment.ibm.SYSTEMP5_back", width: 450.03, height: 178.02, size: 4 },
    "equipment.sangfor.ad2200ls": { label: "equipment.sangfor.ad2200ls", width: 500.02, height: 89.01, size: 2 },
    "equipment.lenovo.system_x3850x6": { label: "equipment.lenovo.system_x3850x6", width: 500.02, height: 178.02, size: 4 },
    "equipment.sangfor.ad2200ls_back": { label: "equipment.sangfor.ad2200ls_back", width: 450.03, height: 89.01, size: 2 },
    "equipment.serva.902": { label: "equipment.serva.902", width: 500.02, height: 89.01, size: 2 },
    "equipment.lenovo.system_x3850x6_back": { label: "equipment.lenovo.system_x3850x6_back", width: 500.02, height: 178.02, size: 4 },
    "equipment.serva.dtst_eqcc": { label: "equipment.serva.dtst_eqcc", width: 500.02, height: 178.02, size: 4 },
    "equipment.inspur.nf560d2": { label: "equipment.inspur.nf560d2", width: 450.03, height: 178.02, size: 4 },
    "equipment.h3c.975": { label: "equipment.h3c.975", width: 500.02, height: 44.49, size: 1 },
    "equipment.huawei.huawei_ar320": { label: "equipment.huawei.huawei_ar320", width: 500.02, height: 89.01, size: 2 },
    "equipment.aether.2733": { label: "equipment.aether.2733", width: 450.03, height: 89.01, size: 2 },
    "equipment.emc.2773": { label: "equipment.emc.2773", width: 500.02, height: 44.49, size: 1 },
    "equipment.aether.2733_back": { label: "equipment.aether.2733_back", width: 450.03, height: 89.01, size: 2 },
    "equipment.h3c.sr6604_x_3": { label: "equipment.h3c.sr6604_x_3", width: 500.02, height: 222.51, size: 5 },
    "equipment.h3c.sr6604_x_3_back": { label: "equipment.h3c.sr6604_x_3_back", width: 450.03, height: 224.13, size: 5 },
    "equipment.hp.dl180_g5": { label: "equipment.hp.dl180_g5", width: 500.38, height: 89.36, size: 2 },
    "equipment.hp.2771": { label: "equipment.hp.2771", width: 450.38, height: 1112.55, size: 25 },
    "equipment.huatie.2819_back": { label: "equipment.huatie.2819_back", width: 450.03, height: 178.02, size: 4 },
    "equipment.hp.dl180_g5_back": { label: "equipment.hp.dl180_g5_back", width: 450.03, height: 89.01, size: 2 },
    "equipment.huatie.2819": { label: "equipment.huatie.2819", width: 500.02, height: 178.02, size: 4 },
    "equipment.hp.2771_back": { label: "equipment.hp.2771_back", width: 450.03, height: 1112.55, size: 25 },
    "equipment.ibm.2785_back": { label: "equipment.ibm.2785_back", width: 450.03, height: 400.53, size: 9 },
    "equipment.lenovo.sr860_2": { label: "equipment.lenovo.sr860_2", width: 500.02, height: 178.02, size: 4 },
    "equipment.lenovo.sr860_2_back": { label: "equipment.lenovo.sr860_2_back", width: 500.02, height: 178.02, size: 4 },
    "equipment.ibm.2785": { label: "equipment.ibm.2785", width: 450.03, height: 405.01, size: 9 },
    "equipment.runpu.rp_rxt6808y_back": { label: "equipment.runpu.rp_rxt6808y_back", width: 450.03, height: 89.01, size: 2 },
    "equipment.runpu.rp_rxt6808y": { label: "equipment.runpu.rp_rxt6808y", width: 500.02, height: 89.01, size: 2 },
    "equipment.zeda.zdc_j101": { label: "equipment.zeda.zdc_j101", width: 500.02, height: 44.49, size: 1 },
    "equipment.sun.2672": { label: "equipment.sun.2672", width: 450.03, height: 133.50, size: 3 },
    "equipment.zeda.zdc_j101_back": { label: "equipment.zeda.zdc_j101_back", width: 450.03, height: 44.49, size: 1 },
    "equipment.sun.2672_back": { label: "equipment.sun.2672_back", width: 450.03, height: 133.32, size: 3 },
    "equipment.lenovo.2842": { label: "equipment.lenovo.2842", width: 165.11, height: 400.04, size: 9 },
    "equipment.lenovo.power_v_214_vpn": { label: "equipment.lenovo.power_v_214_vpn", width: 500.02, height: 44.49, size: 1 },
    "equipment.lenovo.2842_back": { label: "equipment.lenovo.2842_back", width: 165.11, height: 400.04, size: 9 },
    "equipment.lenovo.power_v_214_vpn_back": { label: "equipment.lenovo.power_v_214_vpn_back", width: 450.03, height: 44.49, size: 2 },
    "equipment.sun.sunFire_v440_back": { label: "equipment.sun.sunFire_v440_back", width: 450.03, height: 178.02, size: 4 },
    "equipment.sun.storageTek_6100_back": { label: "equipment.sun.storageTek_6100_back", width: 450.03, height: 133.50, size: 3 },
    "equipment.sun.sunFire_v440": { label: "equipment.sun.sunFire_v440", width: 500.02, height: 178.02, size: 4 },
    "equipment.sun.storageTek_6100": { label: "equipment.sun.storageTek_6100", width: 500.02, height: 133.50, size: 3 },
    "equipment.cisco.3800C_back": { label: "equipment.cisco.3800C_back", width: 450.03, height: 133.50, size: 3 },
    "equipment.cisco.3800C": { label: "equipment.cisco.3800C_front", width: 500.02, height: 133.50, size: 3 },
    "equipment.dell.poweredge_r710_2_back": { label: "equipment.dell.poweredge_r710_2_back", width: 450.03, height: 89.01, size: 2 },
    "equipment.dell.poweredge_r710_2": { label: "equipment.dell.poweredge_r710_2_front", width: 500.02, height: 89.01, size: 2 },
    "equipment.hitachi.2697_back": { label: "equipment.hitachi.2697_back", width: 450.03, height: 89.01, size: 2 },
    "equipment.hp.7200_back": { label: "equipment.hp.7200_back", width: 450.03, height: 89.01, size: 2 },
    "equipment.hitachi.2697": { label: "equipment.hitachi.2697_front", width: 500.02, height: 89.01, size: 2 },
    "equipment.hp.7200": { label: "equipment.hp.7200_front", width: 500.02, height: 89.01, size: 2 },
    "equipment.hp.dl140_back": { label: "equipment.hp.dl140_back", width: 450.03, height: 44.49, size: 1 },
    "equipment.hp.963": { label: "equipment.hp.963_front", width: 500.02, height: 178.02, size: 4 },
    "equipment.hp.hp_rp4440": { label: "equipment.hp.hp_rp4440_front", width: 500.02, height: 178.02, size: 4 },
    "equipment.hp.dl140": { label: "equipment.hp.dl140_front", width: 500.02, height: 44.49, size: 1 },
    "equipment.hp.san": { label: "equipment.hp.san_front", width: 500.02, height: 44.49, size: 1 },
    "equipment.hp.msa60": { label: "equipment.hp.msa60_front", width: 500.02, height: 89.01, size: 2 },
    "equipment.hp.msa60_back": { label: "equipment.hp.msa60_back", width: 450.03, height: 89.01, size: 2 },
    "equipment.huama.077715_back": { label: "equipment.huama.077715_back", width: 450.03, height: 133.50, size: 3 },
    "equipment.huawei.2658_back": { label: "equipment.huawei.2658_back", width: 450.03, height: 44.49, size: 1 },
    "equipment.huawei.huawei_ar320_back": { label: "equipment.huawei.huawei_ar320_back", width: 450.03, height: 89.01, size: 2 },
    "equipment.huawei.2658": { label: "equipment.huawei.2658_front", width: 500.02, height: 44.49, size: 1 },
    "equipment.huama.077715": { label: "equipment.huama.077715_front", width: 500.02, height: 133.50, size: 3 },
    "equipment.huawei.rh2288V3": { label: "equipment.huawei.rh2288V3_front", width: 500.02, height: 90.32, size: 2 },
    "equipment.huawei.rh2288h_v3": { label: "equipment.huawei.rh2288h_v3_front", width: 500.02, height: 89.01, size: 2 },
    "equipment.ibm.ibm_x3650_m3": { label: "equipment.ibm.ibm_x3650_m3_front", width: 500.02, height: 89.01, size: 2 },
    "equipment.ibm.ibm_x3650m2_back": { label: "equipment.ibm.ibm_x3650m2_back", width: 450.03, height: 89.01, size: 2 },
    "equipment.ibm.ibm_x3650m2": { label: "equipment.ibm.ibm_x3650m2", width: 450.03, height: 89.01, size: 2 },
    "equipment.sangfor.1246_back": { label: "equipment.sangfor.1246_back", width: 450.03, height: 89.00, size: 2 },
    "equipment.ibm.3650-M2": { label: "equipment.ibm.3650-M2_front", width: 500.02, height: 90.32, size: 2 },
    "equipment.ibm.ibm_power570_back": { label: "equipment.ibm.ibm_power570_back", width: 450.03, height: 178.02, size: 4 },
    "equipment.cisco.cisco_2960_back": { label: "equipment.cisco.cisco_2960_back", width: 450.03, height: 44.49, size: 1 },
    "equipment.cisco.cisco_3560_back": { label: "equipment.cisco.cisco_3560_back", width: 450.03, height: 44.49, size: 1 },
    "equipment.ibm.x3400_m3": { label: "equipment.ibm.x3400_m3", width: 275.40, height: 445.02, size: 10 },
    "equipment.ibm.x3400_m3_back": { label: "equipment.ibm.x3400_m3_back", width: 273.60, height: 445.02, size: 10 },
    "equipment.ibm.x3850_2_back": { label: "equipment.ibm.x3850_2_back", width: 450.03, height: 133.50, size: 3 },
    "equipment.ibm.x3850_2": { label: "equipment.ibm.x3850_2", width: 500.02, height: 133.50, size: 3 },
    "equipment.ibm.x3850_m2_back": { label: "equipment.ibm.x3850_m2_back", width: 450.03, height: 178.02, size: 4 },
    "equipment.ibm.x3850_m2": { label: "equipment.ibm.x3850_m2", width: 500.02, height: 178.02, size: 4 },
    "equipment.lanbe.as-8108_back": { label: "equipment.lanbe.as-8108_back", width: 500.02, height: 44.49, size: 1 },
    "equipment.lanbe.as-8108": { label: "equipment.lanbe.as-8108", width: 500.02, height: 44.49, size: 1 },
    "equipment.serva.562": { label: "equipment.serva.562", width: 500.02, height: 133.50, size: 3 },
    "equipment.sun.sunfir_280r": { label: "equipment.sun.sunfir_280r", width: 500.02, height: 178.02, size: 4 },
    "equipment.sun.sunfir_280r_back": { label: "equipment.sun.sunfir_280r_back", width: 450.03, height: 178.02, size: 4 },
    "equipment.zte.zxmp_s385": { label: "equipment.zte.zxmp_s385", width: 351.21, height: 890.04, size: 20 },
    "equipment.hp.hp_kvm_back": { label: "equipment.hp.hp_kvm_back", width: 450.03, height: 44.49, size: 1 },
    "equipment.hp.san_back": { label: "equipment.hp.san_back", width: 450.03, height: 44.49, size: 1 },
    "equipment.huawei.hw_rh2288_v3_back": { label: "equipment.huawei.hw_rh2288_v3_back", width: 450.03, height: 89.01, size: 2 },
    "equipment.huawei.oceanstor5600v3_back": { label: "equipment.huawei.oceanstor5600v3_back", width: 450.03, height: 133.50, size: 3 },
    "equipment.huawei.oceanstor5600v3": { label: "equipment.huawei.oceanstor5600v3", width: 500.02, height: 133.50, size: 3 },
    "equipment.h3c.s5820v2": { label: "equipment.h3c.s5820v2", width: 450.03, height: 44.49, size: 1 },
    "equipment.h3c.s5820v2_back": { label: "equipment.h3c.s5820v2_back", width: 450.03, height: 44.49, size: 1 },
    "equipment.h3c.s5820v2-2": { label: "equipment.h3c.s5820v2-2", width: 450.03, height: 44.49, size: 1 },
    "equipment.cisco.02046": { label: "equipment.cisco.02046", width: 361.27, height: 45.16, size: 1 },
    "equipment.potevlo.2989": { label: "equipment.potevlo.2989", width: 500.02, height: 133.50, size: 3 },
    "equipment.potevlo.2988_back": { label: "equipment.potevlo.2988_back", width: 450.03, height: 89.01, size: 2 },
    "equipment.potevlo.2988": { label: "equipment.potevlo.2988", width: 500.02, height: 89.01, size: 2 },
    "equipment.potevlo.2991": { label: "equipment.potevlo.2991", width: 500.02, height: 178.02, size: 4 },
    "equipment.h3c.56800_4C-2": { label: "equipment.h3c.56800_4C", width: 500.02, height: 89.01, size: 2 },
    "equipment.h3c.56800_4C-2_back": { label: "equipment.h3c.56800_4C", width: 500.02, height: 89.01, size: 2 },
    "equipment.h3c.s5820v2-2_back": { label: "equipment.h3c.s5820v2-2_back", width: 450.03, height: 44.49, size: 1 },
    "equipment.potevlo.2989_back": { label: "equipment.potevlo.2989_back", width: 450.03, height: 133.50, size: 3 },
    "equipment.potevlo.2991_back": { label: "equipment.potevlo.2991_back", width: 450.03, height: 178.02, size: 4 },
    "equipment.potevlo.2992_back": { label: "equipment.potevlo.2992_back", width: 450.03, height: 311.52, size: 7 },
    "equipment.potevlo.2993": { label: "equipment.potevlo.2993", width: 500.02, height: 356.01, size: 8 },
    "equipment.potevlo.2993_back": { label: "equipment.potevlo.2993_back", width: 450.03, height: 356.01, size: 8 },
    "equipment.potevlo.2999": { label: "equipment.potevlo.2999", width: 500.02, height: 44.49, size: 1 },
    "equipment.potevlo.2999_back": { label: "equipment.potevlo.2999_back", width: 450.03, height: 44.49, size: 1 },
    "equipment.potevlo.2992": { label: "equipment.potevlo.2992", width: 500.02, height: 311.52, size: 7 },
    "equipment.ship.01899": { label: "equipment.ship.01899", width: 500.02, height: 44.49, size: 1 },
    "equipment.ship.01899_back": { label: "equipment.ship.01899_back", width: 450.03, height: 44.49, size: 1 },
    "equipment.cisco.3550": { label: "equipment.cisco.3550", width: 500.02, height: 44.49, size: 1 },
    "equipment.cisco.3550_back": { label: "equipment.cisco.3550_back", width: 450.03, height: 44.49, size: 1 },
    "equipment1": { label: "1UEQUIPMENT", width: make.Default.getEquipmentWidth() * 10, height: make.Default.getEquipmentHeight(1) * 10, size: 1, svgName: '1u' },
    "equipment2": { label: "2UEQUIPMENT", width: make.Default.getEquipmentWidth() * 10, height: make.Default.getEquipmentHeight(2) * 10, size: 2, svgName: '2u' },
    "equipment3": { label: "3UEQUIPMENT", width: make.Default.getEquipmentWidth() * 10, height: make.Default.getEquipmentHeight(3) * 10, size: 3, svgName: '3u' },
    "equipment4": { label: "4UEQUIPMENT", width: make.Default.getEquipmentWidth() * 10, height: make.Default.getEquipmentHeight(4) * 10, size: 4, svgName: '4u' },
    "equipment5": { label: "5UEQUIPMENT", width: make.Default.getEquipmentWidth() * 10, height: make.Default.getEquipmentHeight(5) * 10, size: 5, svgName: '5u' },
    "equipment6": { label: "6UEQUIPMENT", width: make.Default.getEquipmentWidth() * 10, height: make.Default.getEquipmentHeight(6) * 10, size: 6, svgName: '6u' },
    "equipment7": { label: "7UEQUIPMENT", width: make.Default.getEquipmentWidth() * 10, height: make.Default.getEquipmentHeight(7) * 10, size: 7, svgName: '7u' },
    "equipment8": { label: "8UEQUIPMENT", width: make.Default.getEquipmentWidth() * 10, height: make.Default.getEquipmentHeight(8) * 10, size: 8, svgName: '8u' },
}

//内置的所有2D面板组件
var compDef = {
    "port_96": { label: "port_96", width: 19.76, height: 18.52 },
    ...
    "port_09": { label: "port_09", width: 7.34, height: 12.63 },
    ...
    "port_95": { label: "port_95", width: 13.69, height: 13.69 },
    ...
    "USB_32": { label: "USB_32", width: 17.22, height: 8.61 },
    ...
    "port_37": { label: "port_37", width: 38.7 * 0.3528, height: 30.5 * 0.3528 },
    "port_47": { label: "port_47", width: 39.8 * 0.3528, height: 27.8 * 0.3528 },
    "port_57": { label: "port_57", width: 51 * 0.3528, height: 35.7 * 0.3528 },
}
...
var getIdcIconPath = function (icon) {
    ...
    else if (icon.substr(-4) === '.png') return make.Default.path + 'model/idc/icons/dk/' + icon
    else return make.Default.path + 'model/idc/icons/dk/' + icon + '.png'
}

//获取内置2D面板的svg
var getIdcSVGPath = function (image) {
    if (image.includes('/')) return image;
    else if (image.substr(-4) === '.svg') return make.Default.path + 'model/idc/svg/' + image
    else return make.Default.path + 'model/idc/svg/' + image + '.svg'
}
...
var registerServerBackPanel = function (name, args) {
    ...
    make.Default.register(id, function (json) {
        make.Default.copyProperties({
            imageUrl: args.path ? (args.path + args.filename) : getIdcSVGPath(args.svgName || name),
            client: {
        ...
        modelDefaultParameters: {
            width: {
                name: "宽度",
                value: args.width || make.Default.getEquipmentWidth() * 10, //换算成mm
                type: make.Default.PARAMETER_TYPE_INT,
                propertyType: make.Default.PARAMETER_PROPERTY_TYPE_ACCESSOR
            },
            height: {
                name: "高度",
                value: args.height || make.Default.getEquipmentHeight(args.size || 1) * 10, //换算成mm
                type: make.Default.PARAMETER_TYPE_INT,
                propertyType: make.Default.PARAMETER_PROPERTY_TYPE_ACCESSOR
            }
        },
        category: '面板背板',
        icon: args.path ? (args.path + 'icon.png') : getIdcIconPath(args.label),
        host: true,
    });
}
...
make.Default.register('twaver.idc.component.panel', function (json) {
    ...
    category: '面板部件',
    icon: getIdcIconPath('component.png'),
});
...
var registerServerPanelComp = function (name, args) {
    ...
    make.Default.copy(id, 'twaver.idc.component.panel', function (json) {
        ...
        params.icon = getIdcIconPath(name);
...
/*******************inbuilt  devicePanel *********************/
var getObjModelParameter = function () { return {} }

make.Default.registerModel = function (param) {
    var modelTypeMap = {
        'wrapModel': { paramFunction: getWrapModelDefaultParameters },
        'rackModel': { paramFunction: getRackModelParameter },
        'equipmentModel': { paramFunction: getEquipmentProParameters },
        'objModel': { paramFunction: getObjModelParameter },
        'cardModel': { paramFunction: getCardParameters }
    };
    ...
    var getModelParameters = function (param) {
        return {
            ...
            type: param.type,
            layer: param.layer,
            modelDefaultParameters: modelTypeMap[param.modelType]?modelTypeMap[param.modelType].paramFunction(param):{},
        }
    }

    var modelShortId = param.id.replace(/twaver\.(.*?)\./, '');
    ...
    if (param.modelType == 'objModel') {
        ...
        args.name = fileName;
        args.path = newPath;
        args.parameters = getModelParameters(param);
        args.showShadow = showShadow;
        args.shadowType = shadowType;
        args.merged = merged;
        args.showShadowImage = false;
        args.style = null;
        args.envImage = null;
        args.envOriginImages = null;
        args.singleton = false;
        args.isAbsPath = true;
        args.isGltf = isGltf;
        args.animation = animation;
        args.lod = lod;
        make.Default.registerObj(args);
        ...
        if (param.height > 8) {
            make.Default.copy(param.id, 'twaver.idc.equipment', {
                frontImage: newPath + modelShortId + "_front.jpg",
                backImage: param.backData && (newPath + modelShortId + "_back.jpg"),
                isGltf: param.isGltf
            }, getModelParameters(param));
        } else {
            param.height = Number(param.height) ? Number(param.height) : 1;
            param.width = Number(param.width) ? Number(param.width) : 45;
        ...
        if (param.frontData) {
            var size = (param.frontSvg && param.frontSvg.size) || param.height;
            registerServerBackPanel(modelShortId, {
                size: size,
                label: modelShortId,
                path: make.modelPath + modelShortId + '/',
                filename: modelShortId + '.svg',
                width: (param.frontSvg && param.frontSvg.width) || param.width * 10,
                height: (param.frontSvg && param.frontSvg.height) || make.Default.getEquipmentHeight(size) * 10
            });
        }
        if (param.backData) {
            var size = (param.backSvg && param.backSvg.size) || param.height;
            registerServerBackPanel(modelShortId + '_back', {
                size: (param.backSvg && param.backSvg.size) || param.height,
                label: modelShortId + '_back',
                path: make.modelPath + modelShortId + '/',
                filename: modelShortId + '_back.svg',
                width: (param.backSvg && param.backSvg.width) || param.width * 10,
                height: (param.backSvg && param.backSvg.height) || make.Default.getEquipmentHeight(size) * 10
            });
        }
    } else if (param.modelType == 'cardModel') {
        make.Default.copy(param.id, 'twaver.idc.card', {
            frontImage: newPath + modelShortId + "_front.jpg",
            backImage: param.backData && (newPath + modelShortId + "_back.jpg"),
        }, getModelParameters(param));
    ...
    if (param.category == 'channel') {
        make.Default.copy(param.id + '.top', 'twaver.idc.aisle.top', getRegister(param.width || 50, param.depth || 50, color), getModelParameters(param));
    } else {
...
make.Default.register('twaver.scene.terrain', function (json, callback) {
    ...
    terrain.s({
        'm.side': 'both',
        'm.type': 'phong',
        'm.texture.image': [getSceneImagePath('water.jpg'), getSceneImagePath(json.grassImage)],
        'm.transparent': [false, false],
        'm.opacity': [1, 1],
        'layer0.m.type': 'phong',
        'layer0.m.texturen.image': getSceneImagePath(json.waterImage),
        ...
        'layer1.m.texture.image': getSceneImagePath(json.grassImage),
        'layer1.m.texture1.image': getSceneImagePath(json.grassImage),
...
var registerSceneModel = function(modelJson){
    var showSkybox = modelJson.showSkybox, 
        ...
        terrainHeightMap =  modelJson.terrainHeightMap,
        terrainWaterImage =  modelJson.terrainWaterImage,
        terrainGrassImage =  modelJson.terrainGrassImage,
    ...
    var getCustomSceneParameters = function(){
        ...
        result.terrainHeightMap =  terrainHeightMap,
        result.terrainWaterImage =  terrainWaterImage,
        result.terrainGrassImage =  terrainGrassImage,
    ...
    make.Default.register(id,function (json, callback) {
       ...
       var terrainHeightMap = json.terrainHeightMap;
       var terrainWaterImage = json.terrainWaterImage;
       var terrainGrassImage = json.terrainGrassImage;
       if(showSkybox){
           ...
           if(showTerrain){
               objs.push({id:'twaver.scene.terrain',"width": json.skyboxRadius*2,"depth": json.skyboxRadius*2,heightUnit: Math.ceil(json.skyboxRadius/2000), position:[0,terrainPositionY || -500,0],
                 heightMap:terrainHeightMap, waterImage:terrainWaterImage,grassImage:terrainGrassImage});
           }
       }
       if(json.showLogo){
...
//id, name, path, parameters, showShadow, shadowType,merged, showShadowImage,style, envImage, singleton
...
for(var i in sceneObj){
    ...
    if(buildings && buildings.length > 0){
        for(var j = 0; j < buildings.length; j++){
            ...
            var singleton = true;
            if (building.singleton == false) { //2018-04-09 Kevin,null和undefined是true
                singleton = false;
            }
            if(building.merged == false){
                make.Default.registerObj(building.id, building.name, building.path,{},false,0,false,false,building.style,building.envImage,singleton,false,building.isGltf,building.animation,building.lod,building.envOriginImages,building.flipY); 
            }else{
                make.Default.registerObj(building.id, building.name, building.path, {}, false, 0, true, false, {}, null, singleton,false,building.isGltf,building.animation,building.lod,building.envOriginImages,building.flipY);
            }
        }
    }
    ...
    var envImage = yuanqu.envImage;
    var animation = yuanqu.animation;
    var lod = yuanqu.lod;
    var envOriginImages = yuanqu.envOriginImages;
    if(merged == false){
        make.Default.registerObj(yuanqu.id, yuanqu.name, yuanqu.path,{},false,0,false,false,yuanqu.style, envImage, true,false,yuanqu.isGltf,animation,lod,envOriginImages,yuanqu.flipY); 
    }else{
        make.Default.registerObj(yuanqu.id, yuanqu.name, yuanqu.path,{},false,0,true,false,yuanqu.style, envImage, true,false,yuanqu.isGltf,animation,lod,envOriginImages,yuanqu.flipY); 
    }
    ...
    var terrainHeightMap = scene.terrainHeightMap;
    var terrainWaterImage = scene.terrainWaterImage;
    var terrainGrassImage = scene.terrainGrassImage;
    ...
    registerSceneModel({showSkybox:showSkybox,showTerrain:showTerrain,terrainPositionY:terrainPositionY,terrainHeightMap:terrainHeightMap,terrainWaterImage:terrainWaterImage,terrainGrassImage:terrainGrassImage,skyboxId:skyboxId, skyboxRadius:skyboxRadius,skyboxImage:skyboxImage, skyboxFloorImage:skyboxFloorImage, skyboxPositionY:skyboxPositionY, id:i, 
