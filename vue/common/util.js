...
it.util.mysqlApi = function(tableName,method,data,success,error){
        var url = pageConfig.urlPrex + '/api/mysql/' + tableName.replace('/', '.') + '/' + method;
        $.ajax({
            type: "post",
            contentType: 'application/json; charset=UTF-8',
            url: url,
...
util.createForm = function (props, showSubmit, callback, opt,isadd) {
  ...
  var jsonProps = [];
  var width = parseInt($('#panel_module').css('width')),split = 0 ;
  if(width>1500){
      split = 0.2
  }else if(width>1200){
      split = 0.25
  ...
  $.each(props, function (index, prop) {
      ...
      var label = $('<label for="' + prop.id + '">' + util.i18n(prop.label) + (showColon ? ':' : '') + ' </label>');
      if(isadd===false){
          if(prop.type=="dateSelect"){
              split = 0.5
          }
          row = $('<div class="form-group" style="width:'+split*100+'%;margin-bottom:10px"></div>');
          var lw=label.hide().appendTo($(document.body)).css('width')
          input.css("width",width*split-parseInt(lw)-15+'px')
          label.show();
      } else{
  ...
  if (showSubmit) {       
      ...
      if (!opt.inline) {
          ...
          var btn = $('<button type="submit" class="btn btn-default" style="margin-bottom:10px">' + btnText + '</button>');
          div1.append(btn);
          row.append(div1);
          form.append(row);
      } else {
...
util.getFormData = function (form, jsonProps) {
    ...
    var f = function ($eles) {
        $.each($eles, function (index, ele) {
            ...
            if (!name) return;
            var val = $ele.val(),type=$ele.attr('type');
            if ($ele.attr('type') == 'checkbox') {
                val = $ele.is(':checked');
            }
...
util.createComponent = function (prop, form) {
    ...
    else if (prop.type == 'dateSelect') { //支持上传图片并预览
      input = util.createDateSelect(prop);
    }
    if (!input) return;
    if (prop.id) {
...
util.createPicker = function (prop) {
    var $picker = $('<div class="picker-form-control" data-type="picker" style="margin-left:5px"></div>');
    var opt = prop.params || {};
...
util.createDateSelect= function(prop){
  var inputDiv = $('<div></div>').css('display','inline-block')
  var minInput=$('<input class="form-control w150 date alarm_time_min" name="'+prop.id+'" type="between" placeholder="yyyy-mm-dd hh:ii:ss" value="" />');
  var maxInput=$('<input class="form-control w150 date alarm_time_max" name="'+prop.id+'" type="between" placeholder="yyyy-mm-dd hh:ii:ss">');
  inputDiv.append(minInput);
  inputDiv.append($('<span>-</span>'))
  inputDiv.append(maxInput);
  minInput.datetimepicker({
      ...
      minInput.val(newVal);
      minInput.datetimepicker('update');
      maxInput.datetimepicker('setStartDate', newVal);
  });
  toggleDatePicker(minInput);
  var timeMin = initPlaceholder() + '00:00:00';

  maxInput.datetimepicker({
      zIndex: Math.pow(2, 25),
      weekStart: 1,
      language: 'zh-CN',
      format: 'yyyy-mm-dd hh:ii:ss',
      ...
      var newVal = timeArr[0] + ':' + timeArr[1] + ':' + timeArr[2];
      maxInput.val(newVal);
      maxInput.datetimepicker('update');
      minInput.datetimepicker('setEndDate', maxInput.val());
  });
  toggleDatePicker(maxInput);
  var timeMax = initPlaceholder() + '23:55:00';
    return inputDiv
}
...
util.createSelect = function (prop) {
	...
	if(prop.changefun){
		select.change(util[prop.changefun])
	}
    return select;
}

util.autoCreateOrderId=function(e){
	var val=e.target.value.toLocaleUpperCase();
	var time = new Date().getTime()
	$('#orderId').val(val+time)
}
...
util.modal = function (title, bodyContent, showFooter, showBtnSure, callback, syn) {
    ...
    bodyContent = bodyContent ? $(bodyContent) : $('<p>Nothing</p>');
            ...
            sure.click(function (event) {
                ...
                if (syn && callback) {
                    ...
                    callback(function (msg) {
                        ...
                        layer.close(i);
                        if (error && error.type == 'layer') {
                            var title = error.title || '错误详情';
                    ...
                    if (callback) {
                        var textareaVal = bodyContent.find('textarea').val() == undefined ? '' : bodyContent.find('textarea').val();
                        var open = callback(textareaVal);
                        if (open) {
                            return;
                        }
                        modal.modal('hide');
...
util.modal2 = function(settings, bodyContent, appendedFun, showFooter, showBtnSure, callback) {
    var modal = $('<div class="modal fade"></div>');
...
util.initValidator = function (form, fields, moduleId, isUpdate) {
    var opt = {
        feedbackIcons: {
    ...
    if (!isUpdate && moduleId == 'user') { 
        opt.fields.password.validators.notEmpty = {
            message: '密码不能为空'
        }
        opt.fields.password.validators.callback = {
            message: "密码必需包含大写字母,小写字母,数字,特殊字符中的3种且不能包含用户名",
            callback: function (value, validator,$field) {
                var username = $("#panel_adduser input[name='username']").val();
                if(value.indexOf(username) > -1 || username.indexOf(value) > -1) {
                    return false 
                }
    ...
    if (isUpdate && moduleId == 'user') {
        // opt.fields.password.validators.remote = {
        //     url: '/api/admin/user/login',
        //     message: '密码错误',
        //     type: 'POST',
        //     data: function(validator){
        //       return {
        //         username: $("#panel_updateuser input[name='username']").val(),
        //         password: $("#panel_updateuser input[name='password']").val()
        //       };
        //     }
        // }

        opt.fields.password.validators.callback = opt.fields.newPassword.validators.callback = {
            message: "密码必需包含大写字母,小写字母,数字,特殊字符中的3种且不能包含用户名",
            callback: function (value, validator,$field) {
                var username = $("#panel_updateuser input[name='username']").val();
                if(value.indexOf(username) > -1 || username.indexOf(value) > -1) {
                    return false 
                }
        ...
        opt.fields.confirmPassword.validators.identical = {
            field: 'newPassword',
            message: '两次密码不同请重新输入'
        }
    }
...
util.loadWaitForPutAwayEquipment = function (callback) {
    util.adminApi('equipment_custom', 'search', {status:"waitForPutAway"}, function (data) {
        var map = {};
        data = data || [];
        data.forEach(function (d) {
            map[d.id] = d;
...
util.loadPortsToDataTypeId = function () {
    util.adminApi('template_data', 'findIds', '', function (data) {
        var map = {};
        data = data || [];
        data.forEach(function (d) {
            if (map[d.parentDataTypeId]) {
                map[d.parentDataTypeId][d.id] = true;
            } else {
                map[d.parentDataTypeId] = {};
                map[d.parentDataTypeId][d.id]  = true;
            }
        })
        util.portsToDataTypeId = map;
    })
}

util.loadUsedPortToId = function () {
    util.adminApi('link', 'find', '', function (links) {
        var map = {};
        links = links || [];
        links.forEach(function (link) {
            if (map[link.fromId]) {
                map[link.fromId].push(link.fromPortId);
            } else {
                map[link.fromId] = [link.fromPortId];
            }
            if (map[link.toId]) {
                map[link.toId].push(link.toPortId);
            } else {
                map[link.toId] = [link.toPortId]
            }
        })
        util.usedPortToId = map;
    })
}
...
util.getCurrentRole = function (callback) {
    var url = pageConfig.urlPrex + '/currentuser';
    return $.ajax({
        type: "post",
        contentType: 'application/json; charset=UTF-8',
        url: url,
        success: function (result) {
            var username = result && result.value && result.value.username;
            if (!username) {
                callback && callback()
            } else {
                it.util.adminApi('userOfRole', 'search', {user_id: username}, function (datas) {
                    var roleIds = [];
                    datas.forEach(function (data) {
                        roleIds.push(data.roleId);
                    }) 
                    callback && callback(roleIds);
                })
            }
        },
        error: function (a, b, c) {
...
util.getCurrentUser = function (cb) {
    var url = pageConfig.urlPrex + '/currentuser';
    return $.ajax({
        type: "post",
        contentType: 'application/json; charset=UTF-8',
        ...
        success: function (result) {
            cb && cb(result);
            if(result && result.value && result.value.username){
                it.util.currentusername = result.value.username;
                it.util.initPageCloseLinsten();
            }
        },
        error: function (a, b, c) {
            console.error(a, b, c)
...
util.initPageCloseLinsten = function () {
  var loginoutFn=function(){
    $.ajax({
      type: "post",
      contentType: 'application/json; charset=UTF-8',
      url: pageConfig.urlPrex + '/outline',
      success: function (result) {
  ...
  $(window).on('beforeunload unload',loginoutFn)
}
...
util.timeFormate = function(Date){
  var DateString='';
  var a = Date+'';
  var b =a.slice(16,24);
...
it.util.ShowItemDetail=function(data){
	var content = '<div class="item-detail">';
	for(var index in data){
		content+='<div><span style="width:30%;margin-right:5%">'+it.util.i18n(index)+'</span><span style="width:65%">'+it.util.i18n(data[index])+'</span></div>'
	}
	content+='</div>'
	layer.open({
		type: 1,
