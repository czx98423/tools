...
$(function () {
    ...
    //指定make路径
    make.Default.path = '../static/modellib/';
    ...
    $('#system-menu').on('click', function (e) {
        openSystemMenuPanel();
    });
    ...
    function exportImage() {
        ...
        // if (twaver.Util.isChrome) { //新版的chrome不支持在浏览器上直接输出dataUrl
            sceneEditor.network2d.repaint();
            x = window.open();
            // iframe = x.document.createElement('iframe');
            iframe = x.document.createElement('img');
            // iframe.width = '100%';
            // iframe.height = '100%';
            // iframe.frameBorder = "0px";
            iframe.style = "border:0;width:100%;height:100%";
            x.document.body.appendChild(iframe);
            iframe.setAttribute("src",canvas.toDataURL());
        }
        // else {
    ...
    //     setTimeout(function () {
    //         resetChannelRack();
    //         resetFont();//重新设置一下所有的node的字体样式 Kevin 2018-07-12 
    //     }, 550);
    // });
        ...
        list.forEach(function (item) {
            if (isChannel(item.id)) {
                ...
                if (!racks || racks.length == 0) {
                    ...
                    var itemDataType = it.util.dataTypeMap[item.id];
                    var isPhysicalPosition = isRackUsePhysicalLocation(racks);
                    if (!isPhysicalPosition) {
                        isPhysicalPosition = !itemDataType||!itemDataType.childrenSize||!itemDataType.childrenSize.x;
                    }
                    //如果柜子的尺寸不一致,使用物理坐标
                    if (isPhysicalPosition) {
                        ...
                        racks.forEach(function (rack) {
                            ...
                            rackItem.location = {
                                ...
                                y: null,//'floor-top',
                                // z: loc.y  
                                z:null// 2018-06-25 Kevin 使用物理位置的话逻辑位置应该是null,旋转应该用rotation
                            };
                            rackItem.rotation = {
                                x:0,
                                // y:loc.y%2?0:Math.PI,
                                // y:(rack._angle||0)*Math.PI/180,
                                y:(-1)*(rack._angle-channelNode._angle||0)||0, //cad中的是逆时针的，所以乘以-1
                                z:0
                            }
    ...
    var importFun = function(){
        var content = "<div>" + it.util.i18n("Admin_sceneEditor_Export_data") + ":<br><textarea>123</textarea></div>";

        var modal = util.modal('导入json', content, true, true, function(json){
            if(typeof json == 'string'){
                 ...
                 * 如果floor里没有老门，则正常处理
                 * 如果有，则执行下面几步操作
                 * 1、将存在于floor的datatype里的door对象替换为cutoff
                 * 2、弹出一个页面用于保存旧门的datatype
                 * 3、根据上一步的datatype、旧门的数据去创建新门的数据
                */
                try {
                    transformOldDoorToNew(json);
                    sceneEditor.box.getSelectionModel().clearSelection(); //清空选择,不然在外面粘贴会影响画布
                    var saveSceneDialog = window.saveSceneDialog = new SaveSceneDialog(json, _floorData);
    ...
    function tanslatePosition (pos,pPos){
        if(!pos) return;
        if(!pPos) return;
        pos = [pos[0] - pPos[0], pos[1], pos[2] - pPos[2]];
        return pos;
    }

    function transformOldDoorToNew(datas){
        if(!datas.length) return;
        //找出需要转换的门
        var tanDoors = [];
        var createRandomId = function(str){
            return str + _twaver.id();
        }
        var remToDeg = function(arr){
        ...
        var floorData,floorArr = [],rootId;
        //可能有多个wall
        datas.forEach(function(data){
            var dataId = data.id;
            if(dataId){
                var dataCate = make.Default.getOtherParameter(dataId,'type');
                if(dataCate == 'wall' || dataCate == 'innerWall'){
                    floorArr.push(data);
                    if(data.parentId) rootId = data.parentId;
                    var children = data.children;
                    if(children && children.length){
                        children.forEach(function(child){
                            if(child.id){
                                var childDataCate = make.Default.getOtherParameter(child.id,'type');
                                if(childDataCate == 'door'){
                                    var copyChild = {};
                                    $.extend(true,copyChild,child);
                                     //从cad导入的门的rotation的格式为弧度
                                    //通过编辑器生成的门(这里只能为旧门)的rotation格式为client里的rotaionY
                                    //生成的新门的rotation的格式为角度
                                    //这里的rotation重算,从后台编辑器导出来的rotationY是已经取过反的
                                    
                                    var rotationY;
                                    //编辑器旧门导入
                                    if(copyChild.client && copyChild.client.rotationY){
                                        rotationY = copyChild.client.rotationY;
                                    }
                                    //从cad导入的没有角度的情况
                                    if(!rotationY && copyChild.position && data.data){
                                        data.position = data.position || [0,0,0];
                                        rotationY = getRotationYByDatas(copyChild.position,data.data,data.position);
                                    }
                                    if(rotationY == undefined) rotationY = 0;
                                    copyChild.rotation = [0, rotationY, 0];
                                    tanDoors.push(copyChild);
                                    //将旧门替换为cutoff
                                    child.cid = child.id;
                                    child.doorId = child.objectId ? child.objectId : createRandomId('JsonObject');
                                    child.id = 'twaver.idc.cutoff';
                                    child.objectId = createRandomId('JsonObject');
                                }                             
                            }                           
                        });
                    }
        ...
        if(!floorArr.length) return;
        if(floorArr.length == 1){
            floorData = floorArr[0];
        }else{
            floorData = floorArr.filter(function(fData){
                return fData.objectId == rootId;
            })[0];
        }
        if(!tanDoors.length) return;
        //添加新门
        var modelMap = {};
        tanDoors.forEach(function(tanDoor){
            var id = tanDoor.id;
            tanDoor.modelId = id;
            modelMap[id] = modelMap[id] || 'Datatype:'+id;
            tanDoor.id = modelMap[id];
            if(floorData.position){
                tanDoor.position = tanslatePosition(tanDoor.position,floorData.position);
            }
            tanDoor.parentId = floorData.objectId;
            datas.push(tanDoor);
        })
    }
    ...
    var openScene = function (floorData) {
        ...
        loadChildren(floorData, function (floorChildren) {
            ...
            //重新设置一下所有的node的字体样式 Kevin 2018-07-12  由于上面是异步的，因此来个一秒的延迟
            setTimeout(function(){
                resetFont();  
            },1000);
        });
                ...
                if (node) {
                    ...
                    if (node.getClient('isPhysicalPosition')) {
                        var rks = r.map(function (item) {
                            ...
                            return {
                                x: parseFloat(item.position.x) + w2,
                                y: parseFloat(item.position.z) + h2,
                                width: mp.width || 60,
                                depth: mp.depth || 110,
                                id: item.id
                            }
	...
	//加载场景的时候拿到门将扩展里的宽高保存到make里
        if(make.Default.getOtherParameter(dataTypeId,'type') == 'door'){
            if(data.extend && Object.keys(data.extend)){
                var lenArr = ['width','height','depth'];
                for(var k in data.extend){
                    if(lenArr.indexOf(k) != -1){
                        m[k] = data.extend[k];
                    }
                }
            }
        }
        var dataType = it.util.dataTypeMap[dataTypeId];
        //机房默认不显示编号
        if (dataType.categoryId == it.util.CATEGORY.ROOM) {
            m.client.label = '';
        }
        if (dataType.categoryId == it.util.CATEGORY.CHANNEL) {
            var childrenSize = dataType.childrenSize;
            if (!childrenSize || (!childrenSize.x && !childrenSize.y && !childrenSize.z)) {
                m.client.isPhysicalPosition = true;
            }
        }
    ...
    /**
     * 系统设置，设置好后，将系统设置保存到浏览器的缓存中，使得下次再次进入编辑器时上次设置的依然有效
     */
    var openSystemMenuPanel = function(callback) {
        var editor = sceneEditor;
        var fontSize = localStorage.sceneEditor_fontSize;
        var isWordWrap = localStorage.sceneEditor_isWordWrap;
        ...
        var $form = '<form class="form-horizontal" style="margin-top: 10px;width: 350px;">' +
            ...
            '<label class="col-sm-4 control-label">' + it.util.i18n('TemperatureAndHumidityConfigPanel_Font_Size') + '</label>' +
            '<div class="col-sm-8">' +
            '<select class="form-control" id="fontSize">';
        for(var s = 0 ; s < fonts.length ; s++){
            if (fonts[s] == fontSize) {
                $form = $form + '<option value="'+fonts[s]+'" selected="selected">'+fonts[s]+'</option>';
            }else{
                $form = $form + '<option value="'+fonts[s]+'">'+fonts[s]+'</option>';
            }
        }
        $form  = $form + '</select>' +
            '</div>' +
            '</div>' +
            '<div class="form-group">' +
            '<label class="col-sm-4 control-label">' + it.util.i18n('Word_Wrap') + '</label>' +
            '<div class="col-sm-8">' +
            '<div class="checkbox">' +
            '<label>' +
            '<input type="checkbox" id="wordWrap" '+(isWordWrap=='false'?'':'checked="checked"')+'>' +
            '</label>' +
            '</div>' +
            '</div>' +
            '</div>' +
        ...
        layer.open({
            ...
            yes: function (index, layero) {
                ...
                var isWordWrap = $('#wordWrap').is(':checked');
                // localStorage.sceneEditor_fontSize = fontSize;
                // localStorage.sceneEditor_isWordWrap = isWordWrap;
                // sceneEditor.network2d._box.forEach(function(node){
                //     var type = node.getClient('type');//wall,model,rack,header_rack,area,airCondition,channel
                //     node.setStyle('label.font','bold '+parseInt(fontSize)+'px 微软雅黑,sans-serif');
                //     if (!isWordWrap) {
                //         node.setStyle('label.maxlength',null);
                //     }else{
                //         var length = Math.min(node.getClient('depth')||1000,node.getClient('height')||1000);
                //         node.setStyle('label.maxlength',length);
                //     }
                // });
                resetFont(fontSize,isWordWrap);
                callback && callback();
                layer.close(index);
            },
    ...
    var resetFont = function (fontSize,isWordWrap){
        var saved_fontSize = localStorage.sceneEditor_fontSize;
        var saved_isWordWrap = localStorage.sceneEditor_isWordWrap=="true"||false;
        if (!fontSize) {
            fontSize = saved_fontSize;
        }
        if (isWordWrap == undefined || isWordWrap == null) {
            isWordWrap = saved_isWordWrap;
        }
        if (fontSize != saved_fontSize) {
            localStorage.sceneEditor_fontSize = fontSize;
        }
        if (isWordWrap != saved_isWordWrap) {
            localStorage.sceneEditor_isWordWrap = isWordWrap;
        }
        sceneEditor.network2d._box.forEach(function(node) {
            var type = node.getClient('type'); //wall,model,rack,header_rack,area,airCondition,channel
            //'bold ' + parseInt(fontSize) + 'px 微软雅黑,sans-serif'
            node.setStyle('label.font', ' ' + parseInt(fontSize) + 'px 微软雅黑,sans-serif');
            var width = node._width||0,height = node._height||0,angle = node.getAngle()||0;
            if (angle < 0) {
                angle = angle + 360;
            }
            //来个最优的展示，文字沿着最长的方向，如果height>width,那label旋转90度。2d中node旋转的话，其上面的文字不会跟着旋转
            if (height>width && angle%180 == 0) {
                node.setStyle('label.rotate.angle',90);
            }else if (width >= height && angle > 90 && angle < 270) { //文字就跟着node一起转，并且不让文字朝下 2018-07-16 Kevin
                node.setStyle('label.rotate.angle',angle+180); 
            }else if(width >= height) {
                node.setStyle('label.rotate.angle',angle); //否则文字就跟着node一起转
            }
            if (!isWordWrap) {
                node.setStyle('label.maxlength', null);
            } else {
               var nodeSize = node.getSize();
                // var length = Math.max(node.getClient('depth') || 1000, node.getClient('height') || 1000);
                var length = Math.max(nodeSize.width || 1000, nodeSize.height || 1000);
                node.setStyle('label.maxlength', length);
            }

            if (type == 'channel') { // 内置的几行几列的通道，机柜是画出来的，绘制机柜文字的font
                node.setClient('channel.rackText.font',parseInt(fontSize) + 'px 微软雅黑,sans-serif');//'20px arial'
                node.setClient('channel.rackText.wrap',isWordWrap);
                node.setClient('channel.rackText.rotate',90);
            }
        });
    }
    ...
    var getOpenSceneForm = function (data, items) {
        ...
        var form = util.createForm(props,null,null,null,true);
