...
make.Default.registerObj = function (id, name, path, parameters, showShadow, shadowType, merged, showShadowImage, style, envImage, singleton, isAbsPath,isGltf, animation,lod,envOriginImages) {
    ...
    var fun = function (json, callback) {
        ...
        json.envImage = json.envImage || envImage;
        json.envOriginImages = json.envOriginImages || envOriginImages;
...
// make.Default.change=function(Id,data,callback){
//     var typeId=Id.slice(Id.lastIndexOf('.')+1);
//     var sourceId=Id.replace(/[0-9]+(?=[^0-9]*$)/,'');
    
//     for(var index in modelObj){
//         if(typeId.indexOf(index)==0){
//             var arr=modelObj[index]
//         }
//     }
//     arr.forEach(function(e){
//         if(e.id==typeId){
//             var newData=Object.assign(e,data);
//             make.Default.copy(Id,sourceId,{},{
//                 icon: getIdcIconPath(newData.id + '.png'),
//                 modelDefaultParameters: eval('getComm'+typeId.replace(/[0-9]+(?=[^0-9]*$)/,'').replace(typeId[0],typeId[0].toUpperCase())+'Parameters(newData)')
//             })
//             callback&&callback();
//         }
//     })
...
make.Default.register('twaver.idc.innerWall', function(json, callback) {
    ...
    var insideMap = json.insideMap ? make.Default.getImagePath(json.insideMap) : null;
    var outsideMap = json.outsideMap ? make.Default.getImagePath(json.outsideMap) : null;
    var insideImage = json.insideImage ? getIdcWallImagePath(extend?extend.insideImage:json.insideImage) : null;
    var outsideImage = json.outsideImage ? getIdcWallImagePath(extend?extend.outsideImage:json.outsideImage) : null;
    var position = json.position || [0, 0, 0];
...
make.Default.register('twaver.idc.glassWall', function(json, callback) {
    ...
    var client = json.client || {};
    var width = json.wallDepth;
    var insideImage = getIdcWallImagePath(extend?extend.insideImage:json.insideImage);
    var outsideImage = getIdcWallImagePath(extend?extend.outsideImage:json.outsideImage);
    var repeat = json.repeat || json.wallHeight;
    var position = json.position || [0, 0, 0];
    var showBaseboard = json.showBaseboard;
    var position = json.position || [0, 0, 0];
...
make.Default.register('twaver.idc.rack', function(json, callback) {
    ...
    var frameCutWidth = json.frameCutWidth || 45.5; //机框内部宽度
        var frameCutDepth = json.depth*1.2; //机框内部宽度
    ...
    if (self._objObject[json.name]) {
        ...
        if (!self._objMap[json.name] || self._objMap[json.name].length < 1) {
            ...
            loader.load(obj, mtl, null, function(object) {
                ...
                var depth = json.depth || 70;
                object.setScaleZ(depth / 70);
                object.setScaleX(width/45);
...
for (var i = 1; i < 7; i++) {
    make.Default.registerObj('twaver.idc.guanzi' + i, 'guanzi' + i, make.Default.path + 'model/idc/obj/guanzi' + i + '/', {}, false);
    make.Default.registerObj('twaver.idc.jiekou' + i, 'jiekou' + i, make.Default.path + 'model/idc/obj/jiekou' + i + '/', {}, false);
}
/**
...
var getServerBackPanelParams = function(args) {
    return {
        ...
        category: '面板背板',
        icon: args.path? getIconPath(args) :getIdcIconPath(args.label + '.png'),
        host: true,
    }
}

var getIconPath = function(args){
    var path = args.path;
...
var registerServerBackPanel = function(name, args) {
    ...
    make.Default.register(id, function(json) {
        ...
        make.Default.copyProperties({
            imageUrl: args.path || getIdcSVGPath(name),
            client: {
...
var registerServerBackPanel3D = function(name, args) {
    ...
    make.Default.register(id, function(json, callback) {
        ...
        make.Default.copyProperties({
            ...
            frontImage: args.path || getIdcSVGPath(name),
            style: {
                'front.m.color': 'white',
                'front.m.ambient': 'white'
            }
...
"equipment.serva.eq_9":{label: "equipment.serva.eq_9", width: 500.02, height: 400.53, size: 9},
"equipment.serva.eq_9_back":{label: "equipment.serva.eq_9_back", width: 451.02, height: 401.52, size: 9},
...
var registerDevicePanel = function(name, props, type, svgPath) {
    var height = props.height;
    var id = 'twaver.idc.' + name + '.' + type + '.panel';
    var svgUrl = svgPath + id + '.svg';
    make.Default.register(id, function(json) {
        make.Default.copyProperties({
            imageUrl: svgUrl,
            client: {
        ...
        modelDefaultParameters: {
            width: {
                name: "宽度",
                value: make.Default.getEquipmentWidth() * 10, 
                type: make.Default.PARAMETER_TYPE_INT,
                propertyType: make.Default.PARAMETER_PROPERTY_TYPE_ACCESSOR
            },
            height: {
                name: "高度",
                value: make.Default.getEquipmentHeight(height || 1) * 10, 
                type: make.Default.PARAMETER_TYPE_INT,
                propertyType: make.Default.PARAMETER_PROPERTY_TYPE_ACCESSOR
            }
        },
    ...
    // make.Default.copy('twaver.idc.' + name + '.' + type + '.panel.loader', 'twaver.idc.panel.loader', {data: props[type+'Data']}, {
    //     icon: svgPath + id + '.loader.png',
    //     height: height
    // })
}
...
make.Default.registerModel = function(param) {
    ...
    if (param.modelType == 'objModel') {
        ...
        args.style = null;
        args.envImage = param.envImage;
        args.envOriginImages = param.envOriginImages;
        ...
        // make.Default.registerObj(param.id, fileName, newPath, getModelParameters(param), showShadow, shadowType, merged, false, null, null, false, true, isGltf, animation,lod);
        ...
        var name = modelShortId;
        //注册3D模型
        var eqPath =  param.path + '/' + name + '/';
        // if (param.height > 8) {
            make.Default.copy(param.id, 'twaver.idc.equipment', {
                frontImage: eqPath + name + "_front.jpg",
                backImage: param.backData && (eqPath + name + "_back.jpg")
            }, getModelParameters(param));
        // } else {
        //     param.height = param.height === 0 ? 1 : param.height;
        //     param.width = param.width?param.width:45;
        ...
        //注册2d的前面板
        if (param.frontPanel) {
            registerDevicePanel(name, param, 'front', eqPath);
        }
        //注册2d的后面板
        if (param.backPanel) {
            registerDevicePanel(name, param, 'back', eqPath);
        }

        if(param.frontSvg){
            var svgData = param.frontSvg;
            svgData.label = modelShortId;
            svgData.path = make.modelPath + modelShortId + '/' + svgData.label + '.svg';
            registerServerBackPanel(param.id, svgData);
            registerServerBackPanel3D(param.id, svgData);
        }
        if(param.backSvg){
            var svgData = param.backSvg;
            svgData.label = modelShortId+'_back';
            svgData.path = make.modelPath + modelShortId + '/' + svgData.label + '.svg';
            registerServerBackPanel(param.id+'_back', svgData);
            registerServerBackPanel3D(param.id+'_back', svgData);
        }
...
for(var i in sceneObj){
    ...
    if(buildings && buildings.length > 0){
        for(var j = 0; j < buildings.length; j++){
            ...
            if(building.merged == false){
                make.Default.registerObj(building.id, building.name, building.path,{},false,0,false,false,building.style,building.envImage,singleton,false,building.isGltf,building.animation,building.lod,building.envOriginImages); 
            }else{
                make.Default.registerObj(building.id, building.name, building.path, {}, false, 0, true, false, {}, null, singleton,false,building.isGltf,building.animation,building.lod,building.envOriginImages);
            }
        }
    }
    ...
    if(merged == false){
        make.Default.registerObj(yuanqu.id, yuanqu.name, yuanqu.path,{},false,0,false,false,yuanqu.style, envImage, true,false,yuanqu.isGltf,animation,lod,envOriginImages); 
    }else{
        make.Default.registerObj(yuanqu.id, yuanqu.name, yuanqu.path,{},false,0,true,false,yuanqu.style, envImage, true,false,yuanqu.isGltf,animation,lod,envOriginImages); 
    }
